{% macro build_itg_mds_in_sv_winculum_master() %}
    {% set tablename %}
    {% if target.name=='prod' %}
                inditg_integration.itg_mds_in_sv_winculum_master_temp
            {% else %}
                {{schema}}.inditg_integration__itg_mds_in_sv_winculum_master_temp
    {% endif %}
    {% endset %}
    {% set query %}
