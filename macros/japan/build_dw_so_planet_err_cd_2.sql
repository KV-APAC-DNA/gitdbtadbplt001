{% macro build_dw_so_planet_err_cd_2_temp() %}
    {% set tablename %}
    {% if target.name=='prod' %}
                jpnitg_integration.dw_so_planet_err_cd_2_temp
            {% else %}
                {{schema}}.jpnitg_integration__dw_so_planet_err_cd_2_temp
    {% endif %}
    {% endset %}
    {% set query %}
        CREATE TABLE if not exists
        {% if target.name=='prod' %}
                    jpnitg_integration.dw_so_planet_err_cd_2
                {% else %}
                    {{schema}}.jpnitg_integration__dw_so_planet_err_cd_2
                {% endif %}
