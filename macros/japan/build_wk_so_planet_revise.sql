{% macro build_wk_so_planet_revise_temp() %}
    {% set tablename %}
    {% if target.name=='prod' %}
                jpnwks_integration.wk_so_planet_revise_temp
            {% else %}
                {{schema}}.jpnwks_integration__wk_so_planet_revise_temp
    {% endif %}
    {% endset %}
    {% set query %}
        CREATE TABLE if not exists
        {% if target.name=='prod' %}
                    jpnwks_integration.wk_so_planet_revise
                {% else %}
                    {{schema}}.jpnwks_integration__wk_so_planet_revise
                {% endif %}
