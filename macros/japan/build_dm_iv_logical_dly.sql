...
{% endif %}
{{ log("------------------------------End Query-------------------------------------------------") }}
{% set t_start_query = run_query("SELECT SUBSTRING(MIN(MAIN.YMD_DT),1,10) as ty_tounen_start_dt FROM jpnedw_integration.mt_cld	MAIN WHERE	EXISTS
(SELECT * FROM	jpnedw_integration.mt_cld SUB 
...
{{ log(ty_tounen_start_dt_1) }}
    ...
    {{ log("------------------------------first insert-------------------------------------------------") }}
        ...
        FROM {{ ref('jpnedw_integration__dw_si_sell_in_dly') }} SID
        INNER JOIN {{ ref('jpnedw_integration__edi_item_m') }} ITM ON SID.MATERIAL = ITM.ITEM_CD
    ...
    {{ log(least) }}
        ...
        FROM {{ ref('jpnedw_integration__dw_so_sell_out_dly') }} SPF
        INNER JOIN {{ ref('jpnedw_integration__edi_item_m') }} ITM ON SPF.ITEM_CD = ITM.JAN_CD_SO
  ...
  insert into {% if target.name=='prod' %}
        ...
        {% endif %}
            ...
            SELECT CALDAY,
                ...
                QTY,
                NET_PRC
            FROM {{ ref('jpnedw_integration__dm_iv_logical_dly') }}
     ...
     {{ log("-----------------------------------------insert for inventory_cursor--------------------------------------") }}
       ...
       insert into {% if target.name=='prod' %}
        ...
        FROM (
            ...
            FROM {{ source('jpnedw_integration', 'mt_cld') }} MAIN
            INNER JOIN {{ source('jpnedw_integration', 'mt_cld') }} SUB ON MAIN.YEAR_445 = SUB.YEAR_445
        ...
        LEFT OUTER JOIN (
            ...
            FROM {{ source('jpnedw_integration', 'dw_iv_year_end') }}
               ...
               {% endif %}
					...
					(
						SELECT
							...
							SUM(QTY)		QTY
						FROM
							{{ source('jpnedw_integration', 'dw_iv_year_end') }}
						WHERE
					...
					INNER JOIN
							{{ ref('jpnedw_integration__edi_item_m') }}	ITM
						ON
