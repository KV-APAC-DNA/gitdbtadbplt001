{% macro build_edw_promax_ciw_snapshot() %}
    ...
    {% set query %}
        ...
        (
                                ...
                                px_gts::number(38,7)
                                px_eff_val::float as
                                px_jgf_si_val::float as
                                px_pmt_terms_val::float as
                                px_datains_val::float as
                                px_exp_adj_val::float as
                                px_jgf_sd_val::float as
                                px_ciw_tot::float as
                                px_nts::number(38,7)
        ); 
        create or replace table {{this}} clone {% if target.name=='prod' %}
                ...
                {% else %}
                    {{schema}}.pcfedw_integration__edw_promax_ciw_snapshot
                {% endif %};
        delete from {{this}} where to_date(snapshot_date) < (current_date -105)                              
    {% endset %}
