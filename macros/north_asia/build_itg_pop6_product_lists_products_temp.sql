{% macro build_itg_pop6_product_lists_products_temp() %}
    {% set tablename %}
    {% if target.name=='prod' %}
                ntaitg_integration.itg_pop6_product_lists_products_temp
            {% else %}
                {{schema}}.ntaitg_integration__itg_pop6_product_lists_products_temp
    {% endif %}
    {% endset %}
    {% set query %}
        CREATE TABLE if not exists
        {% if target.name=='prod' %}
                    ntaitg_integration.itg_pop6_product_lists_products
                {% else %}
                    {{schema}}.ntaitg_integration__itg_pop6_product_lists_products
                {% endif %}
    (           cntry_cd varchar(10),
