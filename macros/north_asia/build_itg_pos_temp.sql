{% macro build_itg_pos_temp() %}
    {% set tablename %}
    {% if target.name=='prod' %}
                ntaitg_integration.itg_pos_temp
            {% else %}
                {{schema}}.ntaitg_integration__itg_pos_temp
    {% endif %}
    {% endset %}
    {% set query %}
        CREATE TABLE if not exists
        {% if target.name=='prod' %}
                    ntaitg_integration.itg_pos
                {% else %}
                    {{schema}}.ntaitg_integration__itg_pos
                {% endif %}
    (            pos_dt date,
                vend_cd varchar(40),
                vend_nm varchar(100),
                ...
                brnd_nm varchar(40),
                trim(ean_num) as ean_num varchar(100),
                trim(str_cd) as str_cd varchar(40),
                str_nm varchar(100),
                sls_qty number(18,0),
                sls_amt number(16,5),
