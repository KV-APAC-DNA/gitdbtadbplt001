{% macro update_kr_comm_point_para() %}
    ...
    select case when cnt = 0 then 'FALSE' else 'TRUE' end from (
        select count(*) as cnt 
        from {{source_table}}
        where to_date(ymd_dt) = to_date(CONVERT_TIMEZONE('UTC','Asia/Tokyo',SYSDATE())) 
        and day_of_week = 'Mon' 
        and month_445 <> (
            select month_445 
            from {{source_table}}
            where to_date(ymd_dt) = to_date(CONVERT_TIMEZONE('UTC','Asia/Tokyo',dateadd(day,-1,sysdate() )))
        )
    );
    {% endset %}
    ...
    {% if result == 'TRUE' %}
        ...
        INSERT INTO  {% if target.name=='prod' %}
            ...
            REPLACE(cast(to_date(CONVERT_TIMEZONE('UTC','Asia/Tokyo',dateadd(day,-365,sysdate() ))) as VARCHAR),'-',''),
            REPLACE(cast(to_date(CONVERT_TIMEZONE('UTC','Asia/Tokyo',dateadd(day,-1,sysdate() ))) as VARCHAR),'-',''),
            (select year_445 from {{source_table}} where to_date(ymd_dt) = to_date(CONVERT_TIMEZONE('UTC','Asia/Tokyo',dateadd(day,-1,sysdate() )))),
            ...
            REPLACE(cast(to_date(CONVERT_TIMEZONE('UTC','Asia/Tokyo',SYSDATE())) as VARCHAR),'-',''),
            '2021'
        );
        {%endif%}
    {% endif %}
