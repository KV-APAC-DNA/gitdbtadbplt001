      ...
      UPDATE
		...
		FROM
		 (SELECT
		       ...
		       b.DIHENPINRIYUID
		 FROM
		       DEV_DNA_CORE.JPDCLITG_INTEGRATION.C_TBECINQUIREMEISAI b
		 INNER JOIN 
		 (SELECT
		 ...
		 FROM
		       DEV_DNA_CORE.JPDCLITG_INTEGRATION.C_TBECINQUIREMEISAI MEI
		 INNER JOIN 
		 (SELECT
		 ...
		 FROM
		       DEV_DNA_CORE.JPDCLITG_INTEGRATION.C_TBECINQUIREMEISAI M
		 WHERE   
		       M.DIHENPINKUBUN <> 0 AND  
		       M.DSREN >= (Select dateadd(day,(select CAST(ATTR1 as INTEGER) FROM DEV_DNA_CORE.SNAPJPDCLedw_INTEGRATION.HANYO_ATTR where HANYO_ATTR.KBNMEI = 'DAILYFROM'),CONVERT_TIMEZONE('UTC','Asia/Tokyo',current_timestamp())::date))
		 GROUP BY
		       M.DIINQUIREID ,
		 ...
		 WHERE 
		       MEI.DSREN >= (Select dateadd(day,(select CAST(ATTR1 as INTEGER) FROM DEV_DNA_CORE.SNAPJPDCLedw_INTEGRATION.HANYO_ATTR where HANYO_ATTR.KBNMEI = 'DAILYFROM'),CONVERT_TIMEZONE('UTC','Asia/Tokyo',current_timestamp())::date))
		 GROUP BY
		       MEI.DIINQUIREID ,
