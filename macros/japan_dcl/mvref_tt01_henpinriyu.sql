      ...
      UPDATE
		...
		FROM
		 ...
		 WHERE   
		       ...
		       M.DSREN >= (Select dateadd(day,(select CAST(ATTR1 as INTEGER) FROM DEV_DNA_CORE.SNAPJPDCLedw_INTEGRATION.HANYO_ATTR where HANYO_ATTR.KBNMEI = 'DAILYFROM'),CONVERT_TIMEZONE('Asia/Tokyo',current_timestamp())::date))
		 GROUP BY
		       M.DIINQUIREID ,
		 ...
		 WHERE 
		       MEI.DSREN >= (Select dateadd(day,(select CAST(ATTR1 as INTEGER) FROM DEV_DNA_CORE.SNAPJPDCLedw_INTEGRATION.HANYO_ATTR where HANYO_ATTR.KBNMEI = 'DAILYFROM'),CONVERT_TIMEZONE('Asia/Tokyo',current_timestamp())::date))
		 GROUP BY
		       MEI.DIINQUIREID ,
