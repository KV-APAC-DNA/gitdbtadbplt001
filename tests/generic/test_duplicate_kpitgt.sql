{% test test_duplicate_kpitgt(model,group_by_columns=None,coalesce_columns=None,select_columns=None,need_counts='yes',count_column=None)%}

{% if group_by_columns!=None %}
    {% set filter_logic = [] %}
    {% for item in group_by_columns %}
        {% set filter_logic_item = "upper(trim(" + item + "))" %}
        {% do filter_logic.append(filter_logic_item) %}
    {% endfor %}
    {% set c_pk = "md5(concat(" + filter_logic|join(",'_',") + "))" %} 
            with grouped_by as(
                select 
