...
from (select a.delvry_dt,
             ...
             (select sum(c.nis)
              ...
              and   c.prod_key = a.prod_key) as lst6_mnth_nis,
             (select sum(c.unit_qty)
              from wks_au_metcash_monthly c
              where c.delvry_dt > b.lst6_mnth_delvry_dt
              and   c.delvry_dt <= b.delvry_dt
              and   c.acct_key = a.acct_key
              and   c.prod_key = a.prod_key) as lst6_mnth_qty,
			  (select avg(c.list_price)
              from wks_au_metcash_monthly c
              where c.delvry_dt > b.lst6_mnth_delvry_dt
              and   c.delvry_dt <= b.delvry_dt
              and   c.acct_key = a.acct_key
              and   c.prod_key = a.prod_key) as lst6_mnth_avg_lp
...
select * from wks_au_metcash_lst6_mnth