...
select to_date(a.delvry_dt) as delvry_dt,
             a.acct_key,
             a.prod_key,
             to_date(b.lst_mnth_delvry_dt) as lst_mnth_delvry_dt
...
wks_au_metcash_lst_mnth_dstrbtn_check_prod as (
select delvry_dt,
       acct_key,
       prod_key,
       sum(prod_lst_mnth_qty) as prod_lst_mnth_qty
from (select a.delvry_dt,
             a.acct_key,
             a.prod_key,
             c2.unit_qty as prod_lst_mnth_qty
      from wks_joined a
      left join c2
      on c2.delvry_dt > a.lst_mnth_delvry_dt
...
wks_au_metcash_lst_mnth_dstrbtn_check_acct as (
select delvry_dt,
       acct_key,
       prod_key,
       sum(acct_lst_mnth_qty)  as acct_lst_mnth_qty
from (select a.delvry_dt,
             a.acct_key,
             a.prod_key,
             c1.unit_qty as acct_lst_mnth_qty
      from wks_joined a
        left join c1
      on c1.delvry_dt > a.lst_mnth_delvry_dt
...
select p.delvry_dt::timestamp_ntz(9) as delvry_dt,
       p.acct_key,
       p.prod_key,
       case when p.prod_lst_mnth_qty is null 
        then 0 else p.prod_lst_mnth_qty end as prod_lst_mnth_qty,
       case when a.acct_lst_mnth_qty is null 
        then 0 else a.acct_lst_mnth_qty end as acct_lst_mnth_qty
from wks_au_metcash_lst_mnth_dstrbtn_check_prod p,
    wks_au_metcash_lst_mnth_dstrbtn_check_acct a
