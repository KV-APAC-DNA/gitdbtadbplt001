...
prod_lst6_mnth_qty_query as (select
              a.delvry_dt,
              a.acct_key,
              a.prod_key, 
              count(c.unit_qty) as prod_lst6_mnth_qty
              from wks_au_metcash_monthly c,
              wks_au_metcash_prod_acct_date_comb a,
              wks_au_metcash_dstrbtn_date_range b
              where c.delvry_dt > b.lst6_mnth_delvry_dt
              and   c.delvry_dt <= b.delvry_dt
              and   c.prod_key = a.prod_key
              and   c.unit_qty <> 0
              and a.delvry_dt = b.delvry_dt
              group by 1,2,3
              )
              select * from prod_lst6_mnth_qty_query ,
acct_lst6_mnth_qty_query as (select 
              a.delvry_dt,
              a.acct_key,
              a.prod_key,
              count(c.unit_qty) as acct_lst6_mnth_qty
              from wks_au_metcash_monthly c,
              wks_au_metcash_prod_acct_date_comb a,
              wks_au_metcash_dstrbtn_date_range b
              where c.delvry_dt > b.lst6_mnth_delvry_dt
...
from (select a.delvry_dt,
      ...
      from wks_au_metcash_prod_acct_date_comb a,
           wks_au_metcash_dstrbtn_date_range b,
           prod_lst6_mnth_qty_query,
           acct_lst6_mnth_qty_query
...
select * from wks_au_metcash_lst6_mnth_dstrbtn_check