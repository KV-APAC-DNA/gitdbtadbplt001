...
--final cte
anz_regional_sellout_allmonths  as (
...
FROM (SELECT DISTINCT cntry_cd,
  ...
  LEFT JOIN (select * from wks_anz_base_retail_excellence where  MNTH_ID >= (select last_28mnths from edw_vw_cal_Retail_excellence_Dim)::numeric
...
from anz_regional_sellout_allmonths
)
