...
--final cte
anz_pos_re_sellout_allmonths  as (
SELECT all_months.cntry_cd,
       all_months.sellout_dim_key,
       all_months.month,       
       base.store_count_where_scanned::NUMERIC(38,6) AS actual_stores,
...
FROM (SELECT DISTINCT cntry_cd,
  ...
  LEFT JOIN (select * from wks_anz_pos_base_retail_excellence where  MNTH_ID >= (select last_28mnths from edw_vw_cal_Retail_excellence_Dim)::numeric
...
from anz_pos_re_sellout_allmonths
)
--final select
select * from final
