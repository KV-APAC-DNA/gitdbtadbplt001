...
FROM (SELECT DISTINCT SUBSTRING(base.jj_mnth_id,1,4) AS jj_year,
      ...
      FROM (SELECT DISTINCT *,
            ...
            WHERE msl.market IN ('Australia','New Zealand')
            AND   msl.store_grade = 'A'
            AND   msl.retail_environment IN ('NZ Major Chain Super','Big Box','NZ Pharmacy','AU Major Chain Super')) base
        LEFT JOIN (SELECT DISTINCT distributor_code,
                          ...
                          sku_code,
                          EAN
                   FROM wks_anz_pos_base_retail_excellence
                   WHERE CNTRY_CD IN ('AU','NZ')
                   AND   data_src = 'POS') NOO
        ...
        LEFT JOIN (SELECT DISTINCT EAN,
                          sku_code,
                          COUNTRY_CODE
                   FROM wks_anz_pos_c360_mapped_sku_cd) sku
