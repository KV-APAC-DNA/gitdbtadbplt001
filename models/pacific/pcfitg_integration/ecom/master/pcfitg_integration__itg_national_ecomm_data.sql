{{
    config(
        ...
        delete from {{this}} where week_end_dt in ( select week_end from {{ source('pcfsdl_raw', 'sdl_national_ecomm_data') }} 
        where file_name not in (
        select distinct file_name from {{source('pcfwks_integration','TRATBL_sdl_national_ecomm_data__null_test')}}
    ));
        {%endif%}"
    )
}}
...
with source as(
    ...
    from {{ source('pcfsdl_raw', 'sdl_national_ecomm_data') }}
    where file_name not in (
        select distinct file_name from {{source('pcfwks_integration','TRATBL_sdl_national_ecomm_data__null_test')}}
    ) qualify rnk = 1
),
