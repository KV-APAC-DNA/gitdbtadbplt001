...
vdfa as (
    select
        pac_source_type,
        pac_subsource_type,
        --DATEADD(HOUR, -16, (convert_timezone('UTC', current_timestamp )::TMESTAMPNTZ )::DATE)::date
         '2024-08-11'::date as snap_shot_dt,
        jj_period,
        jj_week_no,
        jj_wk,
        jj_mnth,
...
etd as(
    select
       -- DATEADD(HOUR, -16, (convert_timezone('UTC', current_timestamp )::TIMESTAMPNTZ )::DATE) ::date  
       '2024-08-11'::date as  snapshot_date,
    ...
    edw_time_dim etd where etd.time_id=to_char( '2024-08-11'::date,'YYYYMMDD')::number(18,0) and 
...
vdfaa as (
    ...
    left('2024-08-11'::date+1),10)::date
    ) PROJPRD
...
etdd as(
    ...
    AND ETD.TIME_ID=TO_CHAR( '2024-08-11'::date,'YYYYMMDD')::number(17,0) AND 
    ETDW.JJ_MNTH_ID=ETD.JJ_MNTH_ID AND ETDW.JJ_WK=ETD.JJ_WK
),
union2 as(
    SELECT
