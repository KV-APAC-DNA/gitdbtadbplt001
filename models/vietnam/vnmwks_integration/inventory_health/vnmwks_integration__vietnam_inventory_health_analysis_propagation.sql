...
vietnam_inventory_health_analysis_propagation_prestep as (
    select * from {{ ref('vnmwks_integration__vietnam_inventory_health_analysis_propagation_prestep') }}
),
...
base_data as (
    SELECT CAL.YEAR,
                    ...
                    TRIM(NVL(NULLIF(dist_region, ''), 'NA')) as REGION,
                    TRIM(NVL(NULLIF(province, ''), 'NA')) as ZONE_OR_AREA,
                ...
                FROM INV_SO_SI T1,
                    ...
                    (
                        ...
                        WHERE LTRIM(cust_dim.sap_cust_id, '0') = LTRIM(dist_dim.sap_sold_to_code, '0')
                    ) T5
...
inv as (
    ...
    FROM BASE_DATA
    WHERE cntry_nm || MNTH_ID IN (
            SELECT cntry_nm || MNTH_ID AS INCLUSION
            FROM (
                    SELECT cntry_nm,
                        MNTH_ID,
...
coverage as (
    Select ctry_key,
        ...
        si_inv_db_val,
        Case
            when ctry_key = 'VN' then cast((gts_value * exch_rate) / 1000 as numeric(38, 5))
        end as GTS_USD,
        case
            when ctry_key = 'VN' then cast(
