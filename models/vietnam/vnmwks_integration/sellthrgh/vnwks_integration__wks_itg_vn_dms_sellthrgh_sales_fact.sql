...
wks_vn_dms_sellthrgh_sales_fact as (
  select 
    dstrbtr_id, 
    dstrbtr_type, 
    mapped_spk, 
  ...
  from 
    sdl_vn_dms_sellthrgh_sales_fact 
  where 
    (
      dstrbtr_id, mapped_spk, doc_number, 
      product_code
    ) in (
      select 
        dstrbtr_id, 
...
wks_sdl_vn_dms_sellthrgh_sales_fact as (
  (
    select 
      ...
      curr_date as crtd_dttm 
    from 
      sdl_vn_dms_sellthrgh_sales_fact minus 
    select 
      dstrbtr_id, 
      dstrbtr_type, 
      mapped_spk, 
      ...
      crtd_dttm 
    from 
      wks_vn_dms_sellthrgh_sales_fact
  ) 
  UNION ALL 
  select 
    dstrbtr_id, 
...
select 
  * 
from 
wks_sdl_vn_dms_sellthrgh_sales_fact
