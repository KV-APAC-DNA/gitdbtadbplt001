{{
    config(
        ...
        unique_key=  ['filename'],
        pre_hook= "delete from {{this}} where filename in (select distinct filename from {{ source('vnmsdl_raw', 'sdl_vn_mt_sellout_vinmart') }})"
    )
}}

with source as(
    select * from {{ source('vnmsdl_raw', 'sdl_vn_mt_sellout_vinmart') }}
),
final as(
    select 
