...
(SELECT--- case 1.a: PK present in ITG, record is updated, insert the old record from ITG keeping active as 'N'
       ...
       POS.updt_dttm
FROM (SELECT itg.*,
             ROW_NUMBER() OVER (PARTITION BY sdl.bar_code,sdl.product_id_concung order by sdl.lastchgdatetime::timestamp_ntz(9) ) AS rn
      FROM sdl_mds_vn_price_products sdl,
           {{this}} itg
      WHERE sdl.lastchgdatetime::timestamp_ntz(9) != itg.lastchgdatetime
      AND   sdl.bar_code = itg.bar_code
      AND   coalesce(sdl.product_id_concung , '0')= coalesce(itg.product_id_concung,'0')) POS
WHERE POS.rn = 1
UNION ALL
...
FROM (SELECT sdl.*,
             ROW_NUMBER() OVER (PARTITION BY sdl.bar_code,sdl.product_id_concung order by sdl.lastchgdatetime::timestamp_ntz(9) ) AS rn
      FROM sdl_mds_vn_price_products sdl
      WHERE (nvl(sdl.bar_code,'#'),nvl(sdl.product_id_concung,'#')) NOT IN (SELECT nvl(bar_code,'#') as bar_code,
                                                                 nvl(product_id_concung,'#') as product_id_concung
                                                          FROM {{this}}))pos
WHERE POS.rn = 1) ,
transformed as (SELECT*FROM wks
UNION ALL
SELECT *
FROM {{this}} pos
WHERE (nvl(pos.bar_code,'#'),nvl(pos.product_id_concung,'#')) NOT IN (SELECT nvl(bar_code,'#'),nvl(product_id_concung,'#') FROM wks) 
 ) ,
