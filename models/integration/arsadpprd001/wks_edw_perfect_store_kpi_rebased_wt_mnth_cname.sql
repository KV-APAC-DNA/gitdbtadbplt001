{{
    config(
        materialized='table',
        alias='wks_edw_perfect_store_kpi_rebased_wt_mnth_cname'
    )
}}

with wks_edw_perfect_store_kpi_wt_mnth_cname as
(
    select * from {{ref("wks_edw_perfect_store_kpi_wt_mnth_cname")}}
),

wks_edw_perfect_store_kpi_agg_wt_mnth_cname as
(
    select * from {{ref("wks_edw_perfect_store_kpi_agg_wt_mnth_cname")}}
),
