{{
    config(
        materialized='table',
        alias='wks_edw_perfect_store_kpi_wt_mnth_cname'
    )
}}
...
final as 
(
select country, nvl(customername,'x') as customername, to_char(scheduleddate,'YYYYMM') scheduledmonth, 
kpi, min(cast(kpi_chnl_wt as numeric(8,4)) ) chnl_wt 
