...
wks_indonesia_regional_sellout_base  as(
select * from {{ ref('idnwks_integration__wks_indonesia_regional_sellout_base') }}
),
...
product as(
	SELECT DISTINCT 
		  ...
		  EMD.pka_product_key as pka_product_key,--pka_product_key
		  EMD.pka_product_key_description as pka_product_key_description,--pka_product_key_description
          ...
          -- AND   LTRIM(EMD.MATL_NUM,'0') IN (SELECT DISTINCT LTRIM(MATL_NUM,'0')
                                  ...
                                  -- WHERE sls_org IN (select distinct sls_org from edw_sales_org_dim where ctry_key='ID'))
),
cust as(
	SELECT * FROM (SELECT DISTINCT ECBD.CUST_NUM AS SAP_CUST_ID,
...
transformed as(
	 SELECT  
	   ...
	   TRIM(NVL (NULLIF(SELLOUT.put_up,''),'NA')) AS put_up,
       ...
       CAST('IDR' AS VARCHAR) AS FROM_CURRENCY,
       'USD' AS TO_CURRENCY,
       (C.EXCH_RATE/(C.from_ratio*C.to_ratio))::NUMERIC(15,5) as exchange_rate,--C.EXCH_RATE AS EXCHANGE_RATE,
        SUM(SO_SLS_QTY) SELLOUT_SALES_QUANTITY,
        SUM(SO_SLS_VALUE) AS SELLOUT_SALES_VALUE,
		SUM(SO_SLS_VALUE*(C.EXCH_RATE/(C.from_ratio*C.to_ratio)))::NUMERIC(38,11) AS SELLOUT_SALES_VALUE_USD,--SUM(SO_SLS_VALUE*C.EXCH_RATE) SELLOUT_SALES_VALUE_USD
		...
		SELLOUT.crtd_dttm,
		SELLOUT.updt_dttm
FROM    WKS_INDONESIA_REGIONAL_SELLOUT_BASE  SELLOUT
