...
TRIM(UPPER(EDD.JJ_SAP_DSTRBTR_ID)) as JJ_SAP_DSTRBTR_ID,
EDD.JJ_SAP_DSTRBTR_NM as JJ_SAP_DSTRBTR_NM,
WSPSSF.DSTRBTR_CUST_ID as CUST_ID,
...
'Others' AS CUST_GRP,
EOTD.CHNL as chnl,
EOTD.OUTLET_TYPE as OUTLET_TYPE,
'Others' AS CHNL_GRP,
NULL AS JJID,
...
dateadd(day,-6,convert_timezone('UTC', current_timestamp())) AS CUST_CRTD_DT,
'Others' AS CUST_GRP2,
...
(
  ...
  CUST_ID as CUST_ID,
  DSTRBTR_CUST_NM as CUST_NM,
  DSTRBTR_CUST_ADDR as ADDRESS,
  AREA as CITY,
  ...
  JJID as JJID,
  PST_CD as PST_CD,
  DSTRBTR_CUST_ID_MAP as CUST_ID_MAP,
  DSTRBTR_CUST_NM_MAP as CUST_NM_MAP,
  CHNL_GRP2 as CHNL_GRP2,
  CUST_CRTD_DT as CUST_CRTD_DT,
...
where a.chnl=tiering.channel
and a.outlet_type=tiering.sub_channel
)
select * from final