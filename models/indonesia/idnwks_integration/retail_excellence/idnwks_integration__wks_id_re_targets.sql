...
wks_id_re_targets as (
    select act.jj_mnth_id , 
           act.global_product_brand , 
           case when (tgt.mdp_target <> 0 or tgt.mdp_target is not null) then
           (act.actuals/tgt.mdp_target)*100 
           else 100 end as target_compliance from
   (select jj_mnth_id , global_product_brand, count(*) as actuals from
    ...
    on act.jj_mnth_id between  tgt.start_mnth_id and tgt.end_mnth_id 
