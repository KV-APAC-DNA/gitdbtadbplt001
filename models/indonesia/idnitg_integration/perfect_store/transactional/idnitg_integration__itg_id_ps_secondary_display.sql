{{
    ...
    (
        ...
        pre_hook="{% if is_incremental() %}
                    ...
                    from {{ source('idnsdl_raw', 'sdl_id_ps_secondary_display') }} sdl
                    where sdl.file_name not in (
                    select distinct file_name from {{ source('idnwks_integration', 'TRATBL_sdl_id_ps_secondary_display__null_test') }}
                    union all
                    select distinct file_name from {{ source('idnwks_integration', 'TRATBL_sdl_id_ps_secondary_display__duplicate_test') }}
                    )
                    );
                {% endif %}"
    )
...
with source as (
    ...
    from  {{ source('idnsdl_raw', 'sdl_id_ps_secondary_display') }}
