...
(
    ...
    from {{ source('idnsdl_raw', 'sdl_id_pos_watson_sellout') }}
    where filename not in (
            select distinct file_name from {{ source('idnwks_integration', 'TRATBL_sdl_id_pos_watson_sellout__null_test') }}
            union all
            select distinct file_name from {{ source('idnwks_integration', 'TRATBL_sdl_id_pos_watson_sellout__duplicate_test') }}
			union all
            select distinct file_name from {{ source('idnwks_integration', 'TRATBL_sdl_id_pos_watson_sellout__date_format_test') }}
    ) qualify rnk =1
),
final as
(
