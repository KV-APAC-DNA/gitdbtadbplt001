{{
    ...
    (
        ...
        pre_hook=" delete from {{this}} where dstrbtr_grp_cd||jj_mnth_id in (select distinct dstrbtr_cd||jj_mnth_id from DEV_DNA_CORE.IDNWKS_INTEGRATION.wks_all_distributor_sellout_sales_fact);
...
with source as (
    select * from  DEV_DNA_CORE.IDNWKS_INTEGRATION.wks_all_distributor_sellout_sales_fact
),
final as
(
    select 
