{{
    ...
    (
        ...
        pre_hook=["  delete from {{this}} itg USING DEV_DNA_LOAD.SNAPIDNSDL_RAW.SDL_CSA_RAW_SELLOUT_SALES_FACT sdl where TRIM(UPPER(coalesce(sdl.CABANG, 'NA'))) = TRIM(UPPER(coalesce(itg.Col1, 'NA'))) and TRIM(UPPER(coalesce(sdl.KODECUST, 'NA'))) = TRIM(UPPER(coalesce(itg.Col7, 'NA'))) and TRIM(UPPER(coalesce(sdl.SHIPCODE, 'NA'))) = TRIM(UPPER(coalesce(itg.Col18, 'NA'))) and TRIM(UPPER(coalesce(sdl.KODEBARANG, 'NA'))) = TRIM(UPPER(coalesce(itg.Col2, 'NA'))) and TRIM(UPPER(coalesce(sdl.NO_INV, 'NA'))) = TRIM(UPPER(coalesce(itg.Col5, 'NA'))) and trim(to_date(sdl.TGL_INV)) = trim(itg.Col4) and TRIM(UPPER(itg.dstrbtr_grp_nm)) = TRIM(UPPER('CSA')) and TRIM(UPPER(itg.file_type)) = TRIM(UPPER('Sales'));",
