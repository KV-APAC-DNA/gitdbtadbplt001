...
final as
(
    SELECT TRIM(UPPER(JJ_MNTH_ID||JJ_SAP_DSTRBTR_ID||JJ_SAP_PROD_ID))::VARCHAR(50) AS REC_KEY,
       TRIM(UPPER(JJ_SAP_DSTRBTR_ID||JJ_SAP_PROD_ID))::VARCHAR(50) AS CUST_PROD_CD,
       JJ_MNTH_ID::VARCHAR(10) AS JJ_MNTH_ID,
       TRIM(UPPER(JJ_SAP_DSTRBTR_ID))::VARCHAR(50) AS JJ_SAP_DSTRBTR_ID,
       TRIM(UPPER(JJ_SAP_PROD_ID))::VARCHAR(50) AS JJ_SAP_PROD_ID,
       SUM(SELLIN_QTY)::NUMBER(18,4) AS SELLIN_QTY,
       SUM(SELLOUT_QTY)::NUMBER(18,4) AS SELLOUT_QTY,
       SUM(STRT_INV_QTY)::NUMBER(18,4) AS STRT_INV_QTY,
       SUM(STRT_INV_QTY + SELLIN_QTY - SELLOUT_QTY)::NUMBER(18,4) AS END_INV_QTY,
       SUM(SELLOUT_LAST_TWO_MNTHS_QTY)::NUMBER(18,4) AS SELLOUT_LAST_TWO_MNTHS_QTY,
       SUM(SELLIN_VAL)::NUMBER(18,4) AS SELLIN_VAL,
       SUM(SELLOUT_VAL)::NUMBER(18,4) AS SELLOUT_VAL,
       SUM(STRT_INV_VAL)::NUMBER(18,4) AS STRT_INV_VAL,
       SUM(STRT_INV_VAL + SELLIN_VAL - SELLOUT_VAL)::NUMBER(18,4) AS END_INV_VAL,
       SUM(SELLOUT_LAST_TWO_MNTHS_VAL)::NUMBER(18,4) AS SELLOUT_LAST_TWO_MNTHS_VAL,
       MAX(convert_timezone('UTC', current_timestamp()))::timestamp_ntz AS CRTD_DTTM,
...
SELECT T2.JJ_MNTH_ID,
      ...
      AND   T2.JJ_MNTH_ID = (SELECT CAST(TO_CHAR(TO_DATE(DATEADD ('MONTH',-1,TO_DATE(CAST(JJ_MNTH_ID AS VARCHAR),'YYYYMM'))),'YYYYMM') AS INTEGER)
                             FROM edw_time_dim
                             WHERE TO_DATE(CAL_DATE) = TO_DATE(convert_timezone('UTC', current_timestamp())))
...
SELECT trans_key,
      ...
      AND   T2.JJ_MNTH_ID BETWEEN (SELECT CAST(TO_CHAR(TO_DATE(DATEADD ('MONTH',-3,TO_DATE(CAST(JJ_MNTH_ID AS VARCHAR),'YYYYMM'))),'YYYYMM') AS INTEGER)
                                   FROM edw_time_dim
                                   WHERE TO_DATE(CAL_DATE) = TO_DATE(convert_timezone('UTC', current_timestamp()))) AND (SELECT CAST(TO_CHAR(TO_DATE(DATEADD ('MONTH',-2,TO_DATE(CAST(JJ_MNTH_ID AS VARCHAR),'YYYYMM'))),'YYYYMM') AS INTEGER)
                                                                                FROM edw_time_dim
                                                                                WHERE TO_DATE(CAL_DATE) = TO_DATE(convert_timezone('UTC', current_timestamp())))
      AND   TRIM(UPPER(T1.jj_sap_dstrbtr_id)) NOT IN (SELECT DISTINCT TRIM(UPPER(jj_sap_dstrbtr_id))
                                         FROM edw_distributor_dim
      ...
      GROUP BY (SELECT CAST(TO_CHAR(TO_DATE(DATEADD ('MONTH',-1,TO_DATE(CAST(JJ_MNTH_ID AS VARCHAR),'YYYYMM'))),'YYYYMM') AS INTEGER)
                FROM edw_time_dim
                WHERE TO_DATE(CAL_DATE) = TO_DATE(convert_timezone('UTC', current_timestamp()))),T1.JJ_SAP_DSTRBTR_ID,
               T1.JJ_SAP_PROD_ID
...
SELECT trans_key,
      ...
      GROUP BY (SELECT CAST(TO_CHAR(TO_DATE(DATEADD ('MONTH',-1,TO_DATE(CAST(JJ_MNTH_ID AS VARCHAR),'YYYYMM'))),'YYYYMM') AS INTEGER)
                FROM edw_time_dim
                WHERE TO_DATE(CAL_DATE) = TO_DATE(convert_timezone('UTC', current_timestamp()))),T3.AREA,
               T1.JJ_SAP_PROD_ID
)
