with wks_china_selfcare_regional_sellout_base as
(
    select * from {{ ref('chnwks_integration__wks_china_selfcare_regional_sellout_base') }}
),
...
(
    ...
    (SELECT  
        ...
        CAST('RMB' AS VARCHAR) AS FROM_CURRENCY,
        'USD' AS TO_CURRENCY,
        ...
        SELLOUT.crtd_dttm,
        SELLOUT.updt_dttm
    FROM  wks_china_selfcare_regional_sellout_base  SELLOUT
    ...
    AND b.latest_nts_date = a.nts_date AND b.row_number = 1 AND a.ctry_nm = 'China Selfcare')PROD_KEY1
    ...
    (SELECT a.ctry_nm, a.ean_upc, a.sku, a.pka_productkey, a.pka_productdesc
        ...
        ON a.ctry_nm = b.ctry_nm AND a.ean_upc = b.ean_upc AND a.sku = b.sku
        AND b.latest_nts_date = a.nts_date AND b.row_number = 1 and a.ctry_nm = 'China Selfcare') PROD_KEY2
    ...
    ON   UPPER(SELLOUT.CNTRY_CD) = UPPER(C.cntry_key)  
                
    where  C.FROM_CCY = 'RMB' 
    GROUP BY 
        TIME.YEAR    ,
        TIME.QRTR_NO  ,
        SELLOUT.DAY,
