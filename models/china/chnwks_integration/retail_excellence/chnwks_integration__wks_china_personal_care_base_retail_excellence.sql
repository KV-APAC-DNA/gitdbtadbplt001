--import cte
with edw_rpt_regional_sellout_offtake as (
    select * from {{ source('aspedw_integration', 'edw_rpt_regional_sellout_offtake') }}
),
edw_vw_cal_retail_excellence_dim as (
    select * from {{ source('aspedw_integration', 'edw_vw_cal_retail_excellence_dim') }}
),
cnpc_regional_sellout_mapped_sku_cd as (
    select * from {{ ref('chnwks_integration__wks_china_personal_care_regional_sellout_mapped_sku_cd') }}
),
cnpc_regional_sellout_ean as (
    select * from {{ ref('chnwks_integration__wks_china_personal_care_regional_sellout_ean') }}
),
cnpc_base_retail_excellence
as
(
select  country_code as cntry_cd,
md5(nvl(soldto_code,'sd')||nvl (distributor_code,'dc') ||nvl (store_code,'sc') ||nvl (distributor_name,'dn')||nvl (zone,'zone')||nvl (city,'city')||
nvl (ean,'ean') ||
nvl (product_code,'prdcd')||
nvl (mapped_sku_code,'sku')
||
nvl (store_type,'st') || nvl (store_name,'sn')||nvl (sap_parent_customer_key,'spck') ||nvl (sap_parent_customer_description,'spscd')
|| nvl (sap_customer_channel_key,'scck') ||nvl (sap_customer_channel_description,'sccd') ||nvl (sap_customer_sub_channel_key,'scsck') ||nvl (sap_sub_channel_description,'sscd')
||nvl (customer_segment_key,'csk') ||nvl (customer_segment_description,'csd') || nvl (sap_go_to_mdl_key,'sgmk') ||nvl (sap_go_to_mdl_description,'sgmd') ||nvl (sap_banner_key,'sbk')
||nvl (sap_banner_description,'sbd') ||nvl (sap_banner_format_key,'sbfk') ||nvl (sap_banner_format_description,'sbfd') ||nvl (retail_environment,'re')
|| nvl (global_product_franchise,'gpf') ||nvl (global_product_brand,'gpb') ||nvl (global_product_sub_brand,'gpsb') || nvl (global_product_variant,'gpv')
||nvl (global_product_segment,'gps') ||nvl (global_product_subsegment,'gpss') || nvl (global_product_category,'gpc') ||nvl (global_product_subcategory,'gpsc')
||nvl (global_put_up_description,'gpud') ||nvl (pka_product_key,'pk')||nvl (region,'rg')||nvl (zone,'zone')||nvl(pka_product_key_desc,'pkd')) as sellout_dim_key,
country_name as cntry_nm,
data_source as data_src,
soldto_code,
distributor_code,
ean,
product_code,
distributor_name,
store_code,
store_type,
store_name,
--ean_code,
mapped_sku_code,
msl_product_desc,
year,
mnth_id,
--customer_product_desc,
sap_parent_customer_key,
sap_parent_customer_description,
sap_customer_channel_key,
sap_customer_channel_description,
sap_customer_sub_channel_key,
sap_sub_channel_description,
sap_go_to_mdl_key,
sap_go_to_mdl_description,
sap_banner_key,
sap_banner_description,
sap_banner_format_key,
sap_banner_format_description,
retail_environment,
region,
zone,
city,
customer_segment_key,
customer_segment_description,
global_product_franchise,
global_product_brand,
global_product_sub_brand,
global_product_variant,
global_product_segment,
global_product_subsegment,
global_product_category,
global_product_subcategory,
global_put_up_description,
pka_product_key,
pka_product_key_desc,
sum(sellout_sales_quantity)::DECIMAL(38,6) as sls_qty,
sum(sellout_sales_value)::NUMERIC(38,6) as sls_value,
TRUNC(AVG(SELLOUT_SALES_QUANTITY) :: DECIMAL(38,6)) AS SO_AVG_QTY,
sum(sales_value_list_price)::NUMERIC(38,6) as sales_value_list_price,
SYSDATE()	as crt_dttm	--// SYSDATE
from (select country_code,
country_name,
--sd.soldto_code as soldto_code,
soldto_code,
main.distributor_name||'#'||ltrim(main.distributor_code,'0') as distributor_name,
main.distributor_code,
store_code,
--main.ean,
case when main.mother_code != 'na' and main.mother_code is not null then main.mother_code else main.ean   
end as product_code,
store_type,
main.store_name||'#'||ltrim(main.store_code,'0') as store_name,
case when main.mother_code = 'na' or main.mother_code is null then main.ean else ED.EAN end as ean,
pd.sku_code as mapped_sku_code,
pd.msl_product_desc as msl_product_desc,
data_source,
year,
mnth_id,
sap_parent_customer_key,
sap_parent_customer_description,
sap_customer_channel_key,
sap_customer_channel_description,
sap_customer_sub_channel_key,
sap_sub_channel_description,
sap_go_to_mdl_key,
sap_go_to_mdl_description,
sap_banner_key,
sap_banner_description,
sap_banner_format_key,
sap_banner_format_description,
retail_environment,
region,
zone,
city,
customer_segment_key,
customer_segment_description,
global_product_franchise,
global_product_brand,global_product_sub_brand,
global_product_variant,
global_product_segment,
global_product_subsegment,
global_product_category,
global_product_subcategory,
global_put_up_description,
pka_product_key,
pka_product_key_desc,
sellout_sales_quantity,
sellout_sales_value,
sales_value_list_price
from (select country_code,
country_name,
--max(soldto_code) over (partition by distributor_code order by length(soldto_code) desc,cal_date desc rows between unbounded preceding and --unbounded following) as soldto_code,
soldto_code,
--sd.soldto_code as soldto_code,
max(distributor_name) over (partition by distributor_code order by cal_date desc rows between unbounded preceding and unbounded following) as distributor_name,
distributor_code,
msl_product_code as mother_code,
store_code,
upper(store_type) as store_type,
max(store_name) over (partition by store_code order by cal_date desc rows between unbounded preceding and unbounded following) as store_name,
ean,
msl_product_desc,
data_source,
year as year,
mnth_id as mnth_id,
sap_parent_customer_key,
sap_parent_customer_description,
sap_customer_channel_key,
sap_customer_channel_description,
sap_customer_sub_channel_key,
sap_sub_channel_description,
sap_go_to_mdl_key,
sap_go_to_mdl_description,
sap_banner_key,
sap_banner_description,
sap_banner_format_key,
sap_banner_format_description,
retail_environment,
region,
zone_or_area as zone,
distributor_additional_attribute2 as city,
customer_segment_key,
customer_segment_description,
global_product_franchise,
global_product_brand,
global_product_sub_brand,
global_product_variant,
global_product_segment,
global_product_subsegment,
global_product_category,
global_product_subcategory,
global_put_up_description,
pka_product_key,
upper(pka_product_key_description) as pka_product_key_desc,
sellout_sales_quantity,
sellout_sales_value,
sellout_value_list_price as sales_value_list_price
from edw_rpt_regional_sellout_offtake
where country_name = 'china personal care'
and   data_source in ('sell-out','pos')
and mnth_id >= (select last_28mnths from edw_vw_cal_retail_excellence_dim)::numeric
						   and mnth_id <= (select last_2mnths from edw_vw_cal_retail_excellence_dim)::numeric  
)main
left join cnpc_regional_sellout_ean ED
on --upper(main.pka_product_key_desc)=pd.pka_product_key_description
upper(main.mother_code) = ED.MOTHER_CODE
left join cnpc_regional_sellout_mapped_sku_cd PD
on case when main.mother_code = 'na' then upper(main.ean) else ED.EAN end = PD.ean
)
group by country_code,
country_name,
data_source,
year,
mnth_id,
soldto_code,
distributor_code,
ean,
product_code,
distributor_name,
store_code,
store_name,
store_type,
city,
--customer_product_desc,
sap_parent_customer_key,
sap_parent_customer_description,
sap_customer_channel_key,
sap_customer_channel_description,
sap_customer_sub_channel_key,
sap_sub_channel_description,
sap_go_to_mdl_key,
sap_go_to_mdl_description,
sap_banner_key,
sap_banner_description,
sap_banner_format_key,
sap_banner_format_description,
retail_environment,
region,
zone,
customer_segment_key,
customer_segment_description,
global_product_franchise,
global_product_brand,
global_product_sub_brand,
global_product_variant,
global_product_segment,
global_product_subsegment,
global_product_category,
global_product_subcategory,
global_put_up_description,
--ean_code,
mapped_sku_code,
msl_product_desc,
--sku_description
pka_product_key,
pka_product_key_desc
),

final as 
(
select cntry_cd::varchar as cntry_cd,
       sellout_dim_key::varchar as sellout_dim_key,
       cntry_nm::varchar as cntry_nm,
       data_src::varchar as data_src,
       soldto_code::varchar as soldto_code,
       distributor_code::varchar as distributor_code,
       ean::varchar as ean,
       product_code::varchar as product_code,
       distributor_name::varchar as distributor_name,
       store_code::varchar as store_code,
       store_type::varchar as store_type,
       store_name::varchar as store_type,
       mapped_sku_code::varchar as mapped_sku_code,
       msl_product_desc::varchar as msl_product_desc,
	   year::numeric(18,0) as year,	
	   mnth_id::varchar(23) as mnth_id,
       sap_parent_customer_key::varchar as sap_parent_customer_key,
       sap_parent_customer_description::varchar as sap_parent_customer_description,
       sap_customer_channel_key::varchar as sap_customer_channel_key,
       sap_customer_channel_description::varchar as sap_customer_channel_description,
       sap_customer_sub_channel_key::varchar as sap_customer_sub_channel_key,
       sap_sub_channel_description::varchar as sap_sub_channel_description ,
       sap_go_to_mdl_key::varchar as sap_go_to_mdl_key,
       sap_go_to_mdl_description::varchar as sap_go_to_mdl_description,
       sap_banner_key::varchar as sap_banner_key,
       sap_banner_description::varchar as sap_banner_description,
       sap_banner_format_key::varchar as sap_banner_format_key,
       sap_banner_format_description::varchar as sap_banner_format_description,
       retail_environment::varchar as retail_environment,
       region::varchar as region,
       zone::varchar as zone,
       city::varchar as city,
       customer_segment_key::varchar as customer_segment_key,
       customer_segment_description::varchar as customer_segment_description,
       global_product_franchise::varchar as global_product_franchise,
       global_product_brand::varchar as global_product_brand,
       global_product_sub_brand::varchar as global_product_sub_brand,
       global_product_variant::varchar as global_product_variant,
       global_product_segment::varchar as global_product_segment,
       global_product_subsegment::varchar as global_product_subsegment,
       global_product_category::varchar as global_product_category,
       global_product_subcategory::varchar as global_product_subcategory,
       global_put_up_description::varchar as global_put_up_description,
       pka_product_key::varchar as pka_product_key,
       pka_product_key_desc::varchar as pka_product_key_desc,
	   so_sls_qty::numeric(38,6) AS so_sls_qty,	
	   so_sls_value::numeric(38,6) AS so_sls_value,	
	   so_avg_qty::numeric(38,6) AS so_avg_qty,	
	   sales_value_list_price::numeric(38,12) AS sales_value_list_price,	
	   crt_dttm::timestamp without time zone AS crt_dttm
FROM cnpc_base_retail_excellence
)

select * from final