with  wks_mds_reds_market_msl_target_cnpc as
(
    select * from {{ ref('aspedw_integration__wks_mds_reds_market_msl_target_final') }}
),

cnpc_rpt_retail_excellence_sop as 
 (
 ...
 (
   ...
   round( (total_mdp_target/mds.mdp_target) * 100):: integer as target_compliance   from 
        (select fisc_per,
        ...
        from 
            (select  distinct fisc_per,
                distributor_code,
                 store_code, 
                 global_product_brand
                from cnpc_rpt_retail_excellence_sop
                where   mdp_flag = 'Y'
                ) a
        group by 1,2)b 
    inner join 
    (select * from wks_mds_reds_market_msl_target_cnpc where UPPER(market) = 'CHINA PERSONAL CARE'
     ) mds 
