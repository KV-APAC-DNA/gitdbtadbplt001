...
SELECT SO.CNTRY_CD,
       ...
       CAST(AVG(SLS_QTY) OVER (PARTITION BY cntry_cd,sellout_dim_key ORDER BY month ROWS BETWEEN 2 PRECEDING AND CURRENT ROW) AS DECIMAL(10,2)) AS L3M_AVG_SALES_QTY,
       ...
       CAST(AVG(SLS_QTY) OVER (PARTITION BY cntry_cd,sellout_dim_key ORDER BY month ROWS BETWEEN CURRENT ROW AND 2 following) AS DECIMAL(10,2)) AS F3M_AVG_SALES_QTY
...
f3m_avg_sales_qty::numeric(10,2) AS f3m_avg_sales_qty
from transformation
)
