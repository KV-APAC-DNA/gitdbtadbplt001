...
wks_china_sellout_for_inv_analysis as
(
    select * from {{ ref('chnwks_integration__wks_china_sellout_for_inv_analysis') }}
),
wks_china_inventory_healthy_unhealthy_analysis as
(
    select * from {{ ref('chnwks_integration__wks_china_inventory_healthy_unhealthy_analysis') }} 
),
final as
(
    SELECT 
    ...
    datediff (
        ...
        last_day(
        to_date(
            left(month_year, 4)|| right(month_year, 2), 
            'yyyymm'
        )
        )
    ) diff_weeks, 
    ...
    FROM 
    wks_china_reg_inventory_health_analysis_propagation inv 
    LEFT JOIN (
        select 
        * 
        from 
        wks_china_sellout_for_inv_analysis
    ...
    LEFT JOIN (
        ...
        * 
        from 
        wks_china_inventory_healthy_unhealthy_analysis
