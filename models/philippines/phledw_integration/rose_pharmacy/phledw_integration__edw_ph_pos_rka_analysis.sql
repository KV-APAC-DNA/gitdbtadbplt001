...
,null as pms_nm
    from ph_pos_rka_rose_pharma as pos
     ...
     left   join ( select  item_cd,active,Lst_Price_Unit,max(jj_mnth_id) as jj_mnth_id from  price_list group by 1,2,3) price on (prod.sap_item_cd=price.item_cd and price.jj_mnth_id <= pos.jj_month_id and price.active='Y') 
