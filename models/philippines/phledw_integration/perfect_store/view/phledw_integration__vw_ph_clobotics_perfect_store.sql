...
store as (
    ...
    UNION ALL
    SELECT DISTINCT 'GENERAL TRADE'::character varying AS chnl,
        (
            upper(
                trim(
                    (
...
union_1 as (
    SELECT 'Clobotics' AS dataset,
        ...
        CASE
            WHEN (
                task.value = ((1)::numeric)::numeric(10, 1)
...
(
    SELECT 'Clobotics' AS dataset,
        ...
        CASE
            WHEN (
                store_trans.value = ((0)::numeric)::numeric(18, 0)
            ) THEN 'TRUE'::character varying
            ELSE NULL::character varying
        END AS outofstock,
...
union_4 as (
    SELECT 'Clobotics' AS dataset,
        ...
        CASE
            WHEN (
                (
                    upper((task.task_name)::text) like ('BABY BATH%'::character varying)::text
                )
                AND (
                    upper((task.category)::text) = (
                        'BABY/SKIN HEALTH'::character varying
                    )::text
                )
            ) THEN 'Baby'::character varying
            ELSE task.category
        END AS segment,
        weights.weight AS kpi_chnl_wt,
        targets.target AS mkt_share,
                        ...
                        (
                            ...
                            (
                                    (
                                        ...
                                        FROM itg_ph_clobotics_task task
                                        WHERE 
                                        (
                                                (
                                                    (
                                    ...
                                    JOIN (
                                        SELECT sdl_mds_ph_clobotics_sos_ref.taskname,
