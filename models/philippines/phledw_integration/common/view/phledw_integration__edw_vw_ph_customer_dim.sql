...
WHERE ((edw_customer_sales_dim.sls_org)::TEXT = ('2300'::CHARACTER VARYING)::TEXT)
...
FROM (
edw_customer_base_dim ecbd LEFT JOIN edw_gch_customerhierarchy egch ON (((egch.customer)::TEXT = (ecbd.cust_num)::TEXT))
)
,edw_company_dim ecd
...
(
	(
		(
			(
				(
					edw_customer_sales_dim ecsd LEFT JOIN edw_code_descriptions cddes_pck ON (
							(
...
AND ((ecsd.sls_org)::TEXT = ('2300'::CHARACTER VARYING)::TEXT)
)
)
