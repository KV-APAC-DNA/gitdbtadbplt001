    ...
    with wks as (
        ...
        from (
                ...
                where sdl.lastchgdatetime != itg.last_chg_datetime
                    and trim(replace(sdl.effective_sales_cycle,char(13),'')) = itg.mnth_id
                    and trim(sdl.item_cd) = itg.item_cd
                    and trim(sdl.prefix) = itg.cust_cd
            )
        union all
        select trim(code) as code,
            trim(replace(effective_sales_cycle,char(13),'')) as mnth_id,
        ...
        from (
                ...
                where trim(replace(effective_sales_cycle,char(13),'')) || trim(item_cd) || trim(prefix) not in (
