...
(SELECT DISTINCT rpt_grp_1_desc AS trade_type,
                ...
                parent_cust_cd
        FROM ITG_MDS_PH_REF_PARENT_CUSTOMER WHERE UPPER(trade_type) = 'GENERAL TRADE' AND UPPER(active) = 'Y'),
...
JOIN (select * from LAV	
	 JOIN BASE_SO ON LTRIM(LAV.CUSTOMER_L0,'0') = LTRIM(BASE_SO.SOLDTO_CODE,'0') AND LTRIM(LAV.DISTRIBUTOR_CODE,'0') = LTRIM(BASE_SO.SO_DIST_CD,'0')
	 JOIN GT_CUST ON ltrim(LAV.distributor_code,'0') = ltrim(GT_CUST.Sell_Out_Parent_Customer_L1,'0')
