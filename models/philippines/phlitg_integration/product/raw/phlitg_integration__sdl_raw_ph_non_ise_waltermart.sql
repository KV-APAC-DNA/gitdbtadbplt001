...
with source as(
    ...
    where filename not in (
        ...
        union all
        select distinct file_name from {{SOURCE('phlwks_integration','TRATBL_sdl_ph_non_ise_waltermart__duplicate_test')}}
        union all
        select distinct file_name from {{SOURCE('phlwks_integration','TRATBL_sdl_ph_non_ise_waltermart__test_date_format_odd_eve_leap')}}
        union all
        select distinct file_name from {{SOURCE('phlwks_integration','TRATBL_sdl_ph_non_ise_waltermart__lookup_test')}}
    )
),
final as
(
