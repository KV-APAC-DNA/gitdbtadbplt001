...
wks_philippines_re_basedim_values as (
    select * from {{ ref('phlwks_integration__wks_philippines_re_basedim_values')}}
),
wks_philippines_regional_sellout_act_lm as (
    select * from {{ ref('phlwks_integration__wks_philippines_regional_sellout_act_lm')}}
),
wks_philippines_regional_sellout_act_l3m as (
    select * from {{ ref('phlwks_integration__wks_philippines_regional_sellout_act_l3m')}}
),
wks_philippines_regional_sellout_act_l6m as (
    select * from {{ ref('phlwks_integration__wks_philippines_regional_sellout_act_l6m')}}
),
wks_philippines_regional_sellout_act_l12m as (
    select * from {{ ref('phlwks_integration__wks_philippines_regional_sellout_act_l12m')}}
),
--final cte
...
(
    SELECT RE_BASE_DIM.CNTRY_CD,
       ...
       SYSDATE() AS CRT_DTTM
    FROM WKS_PHILIPPINES_REGIONAL_SELLOUT_BASEDIM BASE_DIM
    LEFT JOIN WKS_PHILIPPINES_RE_BASEDIM_VALUES RE_BASE_DIM
    ...
    LEFT OUTER JOIN (SELECT DISTINCT CNTRY_CD,
                   ...
                   FROM WKS_PHILIPPINES_BASE_RETAIL_EXCELLENCE) CM
    ...
    LEFT OUTER JOIN
--Last Month
    WKS_PHILIPPINES_REGIONAL_SELLOUT_ACT_LM LM
    ...
    LEFT OUTER JOIN
--L3M
    WKS_PHILIPPINES_REGIONAL_SELLOUT_ACT_L3M L3M
    ...
    LEFT OUTER JOIN
--L6M
    WKS_PHILIPPINES_REGIONAL_SELLOUT_ACT_L6M L6M
    ...
    LEFT OUTER JOIN
--L12M
    WKS_PHILIPPINES_REGIONAL_SELLOUT_ACT_L12M L12M
