...
(
    ...
    FROM ( SELECT TARGET.FISC_YR,	
                TARGET.FISC_PER,
                COALESCE(ACTUAL.CNTRY_NM, 'Philippines') AS MARKET,	
                COALESCE(ACTUAL.DATA_SOURCE, TARGET.DATA_SRC) AS DATA_SRC,	
                COALESCE(TARGET.CHANNEL, ACTUAL.CHANNEL) AS CHANNEL_NAME,	
                COALESCE(TARGET.CUSTOMER_L0, ACTUAL.SOLD_TO_CODE) AS SOLD_TO_CODE,	
                COALESCE(TARGET.DISTRIBUTOR_CODE, ACTUAL.DISTRIBUTOR_CODE) AS DISTRIBUTOR_CODE,	
                COALESCE(TARGET.DISTRIBUTOR_NAME, ACTUAL.DISTRIBUTOR_NAME) AS DISTRIBUTOR_NAME,
                COALESCE(TARGET.SELL_OUT_CHANNEL, ACTUAL.CHANNEL) AS SELL_OUT_CHANNEL,		
                COALESCE(TARGET.RETAIL_ENVIRONMENT, ACTUAL.RETAIL_ENVIRONMENT) AS RETAIL_ENVIRONMENT,
                COALESCE(TARGET.STORE_CODE, ACTUAL.STORE_CODE) AS STORE_CODE,	
                COALESCE(TARGET.STORE_NAME, ACTUAL.STORE_NAME) AS STORE_NAME,	
                COALESCE(ACTUAL.STORE_TYPE, 'NA') AS STORE_TYPE,
                COALESCE(TARGET.REGION, ACTUAL.REGION) AS REGION,
                TARGET.STORE_ADDRESS,	
                TARGET.STORE_POSTCODE AS POST_CODE,		
                COALESCE(TARGET."Area/Zone/Province", ACTUAL.ZONE_OR_AREA) AS ZONE_NAME,		
                TARGET.CITY AS CITY,		
                COALESCE(ACTUAL.MASTER_CODE, TARGET.MASTER_CODE) AS PRODUCT_CODE,	
                COALESCE(ACTUAL.MSL_PRODUCT_DESC, TARGET.MSL_PRODUCT_DESC) AS PRODUCT_NAME,	
            ...
            FROM ITG_PH_RE_MSL_LIST TARGET	
            LEFT JOIN (SELECT * FROM WKS_PHILIPPINES_REGIONAL_SELLOUT_ACTUALS) ACTUAL	
                ...
                AND UPPER(LTRIM(TARGET.MASTER_CODE,'0')) = UPPER(LTRIM(ACTUAL.MASTER_CODE,'0'))		
                AND UPPER(LTRIM(TARGET.CUSTOMER_L0,'0')) = UPPER(LTRIM(ACTUAL.SOLD_TO_CODE,'0'))	
            LEFT JOIN (SELECT DISTINCT ITG_MDS_PH_LAV_PRODUCT.ITEM_CD,	
                    ...
                    WHERE UPPER(ITG_MDS_PH_LAV_PRODUCT.ACTIVE::TEXT) = 'Y'::CHARACTER VARYING::TEXT) IMPLP		
                    ON LTRIM (COALESCE(ACTUAL.MAPPED_SKU_CD::TEXT, TARGET.MAPPED_SKU_CD::TEXT),'0'::CHARACTER VARYING::TEXT) = LTRIM (IMPLP.item_cd::TEXT,'0'::CHARACTER VARYING::TEXT)		
    ...
    WHERE FISC_PER <= (SELECT MAX(MNTH_ID)
                    FROM WKS_PHILIPPINES_REGIONAL_SELLOUT_ACTUALS)
),
...
(
    ...
    FROM (SELECT CAST(ACTUAL.YEAR AS numeric(18,0) ) AS YEAR,	
                ...
                COALESCE(ACTUAL.CNTRY_NM, 'Philippines') AS MARKET,		
                COALESCE(ACTUAL.DATA_SOURCE, STORE_DET.DATA_SRC) AS DATA_SRC,
                COALESCE(STORE_DET.CHANNEL, ACTUAL.CHANNEL) AS CHANNEL_NAME,
                COALESCE(STORE_DET.CUSTOMER_L0, ACTUAL.SOLD_TO_CODE) AS SOLD_TO_CODE,		
                COALESCE(STORE_DET.DISTRIBUTOR_CODE, ACTUAL.DISTRIBUTOR_CODE) AS DISTRIBUTOR_CODE,	
                COALESCE(STORE_DET.DISTRIBUTOR_NAME, ACTUAL.DISTRIBUTOR_NAME) AS DISTRIBUTOR_NAME,	
                COALESCE(STORE_DET.SELL_OUT_CHANNEL, ACTUAL.CHANNEL) AS SELL_OUT_CHANNEL,		
                COALESCE(STORE_DET.RETAIL_ENVIRONMENT, ACTUAL.RETAIL_ENVIRONMENT) AS RETAIL_ENVIRONMENT,
                COALESCE(STORE_DET.STORE_CODE, ACTUAL.STORE_CODE) AS STORE_CODE,
                COALESCE(STORE_DET.STORE_NAME, ACTUAL.STORE_NAME) AS STORE_NAME,	
                COALESCE(ACTUAL.STORE_TYPE, 'NA') AS STORE_TYPE,
                COALESCE(STORE_DET.REGION, ACTUAL.REGION) AS REGION,
                STORE_DET.STORE_ADDRESS,	
                STORE_DET.STORE_POSTCODE AS POST_CODE,	
                COALESCE(STORE_DET."Area/Zone/Province", ACTUAL.ZONE_OR_AREA) AS ZONE_NAME,		
                ...
                1 AS TARGET_COMPLAINCE
            FROM (SELECT *
                    FROM WKS_PHILIPPINES_REGIONAL_SELLOUT_ACTUALS A	
                    WHERE NOT EXISTS (SELECT 1
                                FROM ITG_PH_RE_MSL_LIST T		
                                ...
                                AND   UPPER(LTRIM(A.MASTER_CODE,'0')) = UPPER(LTRIM(T.MASTER_CODE,'0'))		
                                AND   UPPER(LTRIM(A.SOLD_TO_CODE,'0')) = UPPER(LTRIM(T.CUSTOMER_L0,'0')))) ACTUAL		
        LEFT JOIN (SELECT DISTINCT GT.Customer_L0,	
                        GT.TRADE_TYPE,	
                        GT.SALES_GROUP,	
                        GT.ACCOUNT_GROUP,	
                        GT.DATA_SRC,		
                        GT.CHANNEL,	
                        GT.DISTRIBUTOR_CODE,	
                        GT.DISTRIBUTOR_NAME|| '#' ||ltrim(GT.distributor_code,'0') AS distributor_name,		
                        GT.PARENT_CUSTOMER,	
                        GT.REGION,	
                        GT."Area/Zone/Province",		
                        GT.CITY,	
                        GT.SELL_OUT_PARENT_CUSTOMER_L2,	
                        GT.SELL_OUT_PARENT_CUSTOMER_L1,	
                        GT.SELL_OUT_CHANNEL,	
                        GT.RETAIL_ENVIRONMENT,	
                        GT.STORE_CODE,	
                        GT.STORE_NAME|| '#' ||ltrim(GT.store_code,'0') AS store_name,	
                        GT.STORE_ADDRESS,	
                        GT.STORE_POSTCODE,	
                        GT.STORE_LAT,		
                        GT.STORE_LONG		
                FROM ( select * from ((SELECT DISTINCT ltrim(cust_id,'0') AS Customer_L0,
                            ...
                            JOIN (SELECT DISTINCT rpt_grp_1_desc AS trade_type,
                                    ...
                                    ON UPPER(LTRIM(A.PARENT_CUSTOMER,'0')) = UPPER(LTRIM(B.PARENT_CUST_CD,'0'))) where C.RNO = 1) GT		
                UNION ALL
                    SELECT DISTINCT MT.Customer_L0,	
                            MT.TRADE_TYPE,	
                            MT.SALES_GROUP,	
                            MT.ACCOUNT_GROUP,		
                            MT.DATA_SRC,	
                            ...
                            MT.PARENT_CUSTOMER,		
                            MT.REGION,	
                            MT."Area/Zone/Province",		
                            MT.CITY,	
                            MT.SELL_OUT_PARENT_CUSTOMER_L2,		
                            MT.SELL_OUT_PARENT_CUSTOMER_L1,		
                            MT.SELL_OUT_CHANNEL,		
                            MT.RETAIL_ENVIRONMENT,		
                            MT.STORE_CODE,		
                            MT.STORE_NAME|| '#' ||ltrim(MT.store_code,'0') AS store_name,	
                            MT.STORE_ADDRESS,		
                            MT.STORE_POSTCODE,		
                            MT.STORE_LAT,	
                            MT.STORE_LONG	
                    FROM ( select * from ((SELECT DISTINCT ltrim(cust_id,'0') AS Customer_L0,
                                ...
                                JOIN (SELECT DISTINCT SOLDTO_CODE,
                                        ...
                                        FROM WKS_PHILIPPINES_BASE_RETAIL_EXCELLENCE
                                        WHERE DATA_SOURCE IN ('POS','STOCK TRANSFER')) REG_SO
                                ON LTRIM(A.CUSTOMER_L0,'0') = LTRIM(REG_SO.SOLDTO_CODE,'0')		
                                ...
                                JOIN (SELECT DISTINCT rpt_grp_1_desc AS trade_type,
                                    ...
                                    WHERE UPPER(TRADE_TYPE) IN ('NATIONAL KEY ACCOUNT', 'RKA AND RESELLER') AND UPPER(ACTIVE) = 'Y') B 
                                    ON UPPER(LTRIM(A.PARENT_CUSTOMER,'0')) = UPPER(LTRIM(B.PARENT_CUST_CD,'0')))where C.RNO = 1) MT) STORE_DET		
                                    ON LTRIM(STORE_DET.STORE_CODE,'0') = LTRIM(ACTUAL.STORE_CODE,'0')		
                                    AND LTRIM(STORE_DET.DISTRIBUTOR_CODE,'0') = LTRIM(ACTUAL.DISTRIBUTOR_CODE,'0')		
                                    AND LTRIM(STORE_DET.CUSTOMER_L0,'0') = LTRIM(ACTUAL.SOLD_TO_CODE,'0')		
            LEFT JOIN (SELECT DISTINCT ITG_MDS_PH_LAV_PRODUCT.ITEM_CD,		
                            ITG_MDS_PH_LAV_PRODUCT.ITEM_NM,		
                            ITG_MDS_PH_LAV_PRODUCT.SCARD_FRANCHISE_DESC,
                            ITG_MDS_PH_LAV_PRODUCT.SCARD_BRAND_DESC,		
                    ...
                    FROM ITG_MDS_PH_LAV_PRODUCT		
                    WHERE UPPER(ITG_MDS_PH_LAV_PRODUCT.ACTIVE::TEXT) = 'Y'::CHARACTER VARYING::TEXT) IMPLP		
                    ON LTRIM (ACTUAL.MAPPED_SKU_CD::TEXT,'0'::CHARACTER VARYING::TEXT) = LTRIM (IMPLP.ITEM_CD::TEXT,'0'::CHARACTER VARYING::TEXT)
