...
dist_details as(
    SELECT *
	FROM (SELECT DISTINCT LTRIM(DISTRIBUTOR_CODE,'0') AS DIST_CODE,
                DISTRIBUTOR_NAME,
				ROW_NUMBER() OVER (PARTITION BY LTRIM(DIST_CODE,0) ORDER BY cal_date DESC,LENGTH(DISTRIBUTOR_NAME) DESC) AS RNO
...
FROM (SELECT COUNTRY_CODE,
             ...
             MNTH_ID,
			 MAX(CASE
             WHEN LTRIM(SOLDTO_CODE,'0') = 'NA' THEN '-1'
             ELSE LTRIM(SOLDTO_CODE,'0')
             END) OVER (PARTITION BY LTRIM(DISTRIBUTOR_CODE,'0') ORDER BY cal_date DESC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS SOLDTO_CODE,
...
LEFT JOIN dist_details DD ON  LTRIM (MAIN.DISTRIBUTOR_CODE,'0') = LTRIM (DD.DIST_CODE,'0')
