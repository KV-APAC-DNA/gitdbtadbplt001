...
dist_details as(
    SELECT *
	FROM (SELECT
                LTRIM(DISTRIBUTOR_CODE,'0') AS DIST_CODE,
                DISTRIBUTOR_NAME,
                LTRIM(STORE_CODE,'0') AS STR_CD,
                STORE_NAME,
				ROW_NUMBER() OVER (PARTITION BY LTRIM(DIST_CODE,0), LTRIM(STR_CD,'0')  ORDER BY cal_date DESC, LENGTH(DISTRIBUTOR_NAME) DESC) AS RNO
...
soldto_details as(
    SELECT *
	FROM (SELECT DISTINCT LTRIM(SOLDTO_CODE,'0') AS SOLD_TO_CODE,
                LTRIM(DISTRIBUTOR_CODE,'0') AS DIST_CODE,
				ROW_NUMBER() OVER (PARTITION BY LTRIM(SOLDTO_CODE,0)  ORDER BY cal_date DESC, LENGTH(SOLDTO_CODE) DESC) AS RNO
		  FROM  EDW_RPT_REGIONAL_SELLOUT_OFFTAKE 
		  WHERE COUNTRY_CODE = 'PH'
		  AND   DATA_SOURCE IN ('SELL-OUT','POS','STOCK TRANSFER')
          AND   MNTH_ID >= (SELECT last_27mnths
...
SELECT COUNTRY_CODE AS CNTRY_CD,
       ...
       LTRIM(SD.SOLD_TO_CODE,'0') AS SOLDTO_CODE,
	   DISTRIBUTOR_CODE,
	   DD.DISTRIBUTOR_NAME|| '#' ||ltrim(DISTRIBUTOR_CODE,'0') AS DISTRIBUTOR_NAME,
       STORE_CODE,
       DD.STORE_NAME|| '#' ||ltrim(STORE_CODE,'0') AS STORE_NAME,
       STORE_TYPE,
	   CHANNEL,
...
FROM (SELECT COUNTRY_CODE,
             ...
             --MAX(STORE_NAME) OVER (PARTITION BY LTRIM(DISTRIBUTOR_CODE,'0'), LTRIM(STORE_CODE,'0') ORDER BY cal_date DESC ROWS BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING) AS STORE_NAME,
             STORE_TYPE,
			 CHANNEL,
             SAP_PARENT_CUSTOMER_KEY,
             SAP_PARENT_CUSTOMER_DESCRIPTION,
...
LEFT JOIN dist_details DD ON  LTRIM (MAIN.DISTRIBUTOR_CODE,'0') = LTRIM (DD.DIST_CODE,'0') AND LTRIM (MAIN.STORE_CODE,'0') = LTRIM (DD.STR_CD,'0')
LEFT JOIN soldto_details SD ON LTRIM (MAIN.SOLDTO_CODE,'0') = LTRIM (SD.SOLD_TO_CODE,'0') 
--AND LTRIM (MAIN.DISTRIBUTOR_CODE,'0') = LTRIM (SD.DIST_CODE,'0') 
