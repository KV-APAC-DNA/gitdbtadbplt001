			...
			CASE 
				WHEN C_TBECINQUIREMEISAI.C_DSPOINTITEMFLG = '0'
					THEN decode(C_TBECINQUIREMEISAI.DITOTALPRC, 0, 0, - 1 * CEIL((
									...
									) / trunc(((DECODE(C_TBECINQUIREMEISAI.DIITEMPRC, 0, 1, (cast(C_TBECINQUIREMEISAI.DITOTALPRC AS DECIMAL(10, 2)))::number(38,20) / cast(C_TBECINQUIREMEISAI.DIITEMPRC AS DECIMAL(10, 2))))),11) --消費税率
							))
						--ポイント交換商品
				ELSE - 1 * CEIL((
			...
			CASE 
				WHEN C_TBECINQUIREMEISAI.C_DSPOINTITEMFLG = '0'
					THEN DECODE(C_TBECINQUIREMEISAI.DITOTALPRC, 0, 0, - 1 * CEIL((
									...
									) / trunc(((DECODE(C_TBECINQUIREMEISAI.DIITEMPRC, 0, 1, (cast(C_TBECINQUIREMEISAI.DITOTALPRC AS DECIMAL(10, 2)))::number(38,20) / cast(C_TBECINQUIREMEISAI.DIITEMPRC AS DECIMAL(10, 2))))),11) --消費税率 --消費税率
