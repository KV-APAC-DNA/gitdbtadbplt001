...
AS (
	SELECT *
	FROM DEV_DNA_CORE.SNAPJPDCLEDW_INTEGRATION.CIR01KOKYA
	)
	,cim05opera
AS (
	SELECT *
	FROM DEV_DNA_CORE.SNAPJPDCLEDW_INTEGRATION.CIM05OPERA
	)
	,cim04bumon
AS (
	SELECT *
	FROM DEV_DNA_CORE.SNAPJPDCLEDW_INTEGRATION.CIM04BUMON
	)
	,tm58rireki_nm
AS (
	SELECT *
	FROM DEV_DNA_CORE.SNAPJPDCLEDW_INTEGRATION.TM58RIREKI_NM
	)
