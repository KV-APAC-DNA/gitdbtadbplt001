...
SELECT
		...
		FROM
		 ...
		 WHERE   
		       ...
		       M.DSREN >=  (Select dateadd(day,(select CAST(ATTR1 as INTEGER) FROM HANYO_ATTR where HANYO_ATTR.KBNMEI = 'DAILYFROM'),CONVERT_TIMEZONE('UTC','Asia/Tokyo',SYSDATE())::date))
		 GROUP BY
		       M.DIINQUIREID ,
		 ...
		 WHERE 
		       MEI.DSREN >= (Select dateadd(day,(select CAST(ATTR1 as INTEGER) FROM HANYO_ATTR where HANYO_ATTR.KBNMEI = 'DAILYFROM'),CONVERT_TIMEZONE('UTC','Asia/Tokyo',SYSDATE())::date))
		 GROUP BY
		       MEI.DIINQUIREID ,
