with KR_FREQUENCY_ALL_ORD_WORK_kizuna as (
select * from DEV_DNA_CORE.JPDCLEDW_INTEGRATION.KR_FREQUENCY_ALL_ORD_WORK_KIZUNA
),
KR_FREQUENCY_ALL_ORD_WORK_kizuna as (
select * from DEV_DNA_CORE.JPDCLEDW_INTEGRATION.KR_FREQUENCY_ALL_ORD_WORK_KIZUNA
),
transformed as (
SELECT A.SALENO                AS SALENO       --QlikViewに合わせる
      ,A.KOKYANO               AS KOKYANO
      ,A.ROWNO                 AS NOW_ROWNO    --顧客別受注No_定期以外全件
      ,B.ROWNO                 AS PRE_ROWNO
      ,A.JUCHDATE              AS NOW_JUCHDATE
      ,B.JUCHDATE              AS PRE_JUCHDATE
      ,datediff(day,B.JUCHDATE, A.JUCHDATE) AS ELAPSED      --経過日数_定期以外全件
...
FROM   KR_FREQUENCY_ALL_ORD_WORK_kizuna A
       LEFT JOIN KR_FREQUENCY_ALL_ORD_WORK_kizuna B
