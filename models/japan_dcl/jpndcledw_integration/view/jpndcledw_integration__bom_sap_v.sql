...
FROM (
    SELECT DISTINCT matl1.old_matl_num AS oya_hin_cd,
        ...
        trunc(((bom.quantity / bom.basequantity)),4)::NUMERIC(38, 8) AS kos_qut
    FROM (
        (
            ...
            (
                (
                    (
                        ...
                        AND (bom.validfrom <= to_date(convert_timezone('Asia/Singapore'::TEXT, current_timestamp()::date::TIMESTAMP without TIME zone)))
                        )
                    AND (bom.validto >= to_date(convert_timezone('Asia/Singapore'::TEXT, current_timestamp()::date::TIMESTAMP without TIME zone)))
                    )
                AND (bom.validfrom_zvlfromc <= to_date(convert_timezone('Asia/Singapore'::TEXT, current_timestamp()::date::TIMESTAMP without TIME zone)))
                )
            AND (bom.validto_zvltoi >= to_date(convert_timezone('Asia/Singapore'::TEXT, current_timestamp()::date::TIMESTAMP without TIME zone)))
            )
    ) base1
WHERE (
        (
