...
AS (
    SELECT *
    FROM dev_dna_core.snapjpdcledw_integration."wqtm07属性未設定名称マスタ"
    ),
c_tbecclient
AS (
    SELECT *
...
ct1
AS (
    SELECT TRUNC(h.shukadate / 100) AS shukanengetu,
        CASE 
            WHEN h.smkeiroid = 6
                THEN '02'
            ELSE '01'
    ...
    WHERE h.cancelflg = 0
        ...
        AND h.kakokbn = 0
    GROUP BY TRUNC(h.shukadate / 100),
        CASE 
            WHEN h.smkeiroid = 6
                THEN '02'
            ELSE '01'
...
AS (
    ...
    SELECT trunc((h.shukadate)::DOUBLE PRECISION / 100) AS shukanengetu,
        '11'::VARCHAR AS channel,
        ''::VARCHAR AS konyuchubuncode,
        h.torikeikbn,
        h.tokuicode,
        m.itemcode,
        k.kosecode,
        h.juchkbn,
        sum((m.suryo * COALESCE(k.suryo, 1::NUMERIC(18, 0)))) AS suryo,
        sum(CASE 
                WHEN (
                        SUBSTRING(h.juchkbn, 1, 1) = '9'
                        OR (
                            SUBSTRING(h.juchkbn, 1, 1) IS NULL
                            AND '9' IS NULL
                            )
                        )
                    THEN (m.hensu * COALESCE(k.suryo, 1::NUMERIC(18, 0)))
                ELSE 0::NUMERIC(18, 0)
                END) AS hensu,
        0 AS anbunmaegaku_tyouseimae,
        0 AS meisainukikingaku_tyouseimae,
        sum(m.meisainukikingaku) AS anbunmaegaku_tyouseigo,
        sum(m.meisainukikingaku * COALESCE(k.koseritsu, 1::NUMERIC(18, 0))) AS meisainukikingaku_tyouseigo,
        sum(m.anbunmeisainukikingaku * COALESCE(k.koseritsu, 1::NUMERIC(18, 0))) AS anbunmeisainukikingaku,
        h.henreasoncode,
        h.henreasonname,
        ''::VARCHAR AS sokoname
    ...
    LEFT JOIN tm14shkos k ON m.itemcode = k.itemcode
    WHERE h.kakokbn = 0::NUMERIC(18, 0)
    GROUP BY trunc((h.shukadate)::DOUBLE PRECISION / 100),
        h.torikeikbn,
        h.tokuicode,
        m.itemcode,
        k.kosecode,
...
AS (
    SELECT t.shukanengetu,
        ...
        t.konyuchubuncode,
        t.torikeikbn,
        t.tokuicode || NVL2(cim02.tokuiname, ' : ', '') || COALESCE(cim02.tokuiname || ' ' || cim02.tokuiname_ryaku, '') AS tokuicode,
        COALESCE(t.itemcode, '') || ' : ' || COALESCE(COALESCE(ia.itemname, ''), COALESCE(ia2.itemname, '')) AS hanbaiitem,
        ib.bar_cd2 || ' : ' || COALESCE(ib.itemname, ib2.itemname) AS kouseiitem,
        t.kosecode,
        ...
        t.henreasoncode,
        t.henreasoncode || ' : ' || t.henreasonname AS henreasonname,
        t.sokoname,
