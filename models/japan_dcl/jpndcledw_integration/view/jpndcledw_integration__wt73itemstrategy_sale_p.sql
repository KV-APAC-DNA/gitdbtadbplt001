...
(
    SELECT *
    FROM JPDCLEDW_INTEGRATION.cim03item_hanbai
),
CIM03ITEM_ZAIKO
AS
(
    SELECT *
    FROM JPDCLEDW_INTEGRATION.CIM03ITEM_ZAIKO
),
ZAIKO_SHOHIN_ATTR
AS
(
    SELECT *
    FROM JPDCLEDW_INTEGRATION.ZAIKO_SHOHIN_ATTR
),
TM67JUCH_NM
AS
(
    SELECT *
    FROM JPDCLEDW_INTEGRATION.TM67JUCH_NM
),
CIM02TOKUI
AS
(
    SELECT *
    FROM JPDCLEDW_INTEGRATION.CIM02TOKUI
),
tm14shkos
AS (
    SELECT *
    FROM JPDCLEDW_INTEGRATION.tm14shkos
    ),
CIM24ITBUN AS
(
    SELECT *
    FROM JPDCLEDW_INTEGRATION.CIM24ITBUN
),
cit86osalm
AS (
    SELECT *
    FROM JPDCLEDW_INTEGRATION.cit86osalm
    ),
c_tbecclient
AS (
    SELECT *
    FROM JPDCLITG_INTEGRATION.c_tbecclient
    ),
cit85osalh_kaigai
AS (
    SELECT *
    FROM JPDCLEDW_INTEGRATION.cit85osalh_kaigai
    ),
cit86osalm_kaigai
AS (
    SELECT *
    FROM JPDCLEDW_INTEGRATION.cit86osalm_kaigai
    ),
"wqtm07属性未設定名称マスタ"
AS (
    SELECT *
    FROM JPDCLEDW_INTEGRATION."wqtm07属性未設定名称マスタ"
    ),
get_ci_next_sale
AS (
    SELECT *
    FROM JPDCLEDW_INTEGRATION.get_ci_next_sale
    ),
cit80saleh
AS (
    SELECT *
    FROM JPDCLEDW_INTEGRATION.cit80saleh
    ),
cit81salem
AS (
    SELECT *
    FROM JPDCLEDW_INTEGRATION.cit81salem
    ),
cim08shkos_bunkai
AS (
    SELECT *
    FROM JPDCLEDW_INTEGRATION.cim08shkos_bunkai
    ),
syouhincd_henkan
AS (
    SELECT *
    FROM JPDCLEDW_INTEGRATION.syouhincd_henkan
    ),
t1
AS (
    SELECT h.saleno,
...
AS (
    ...
    GROUP BY t.shukanengetu,
        ...
        (((COALESCE(t.itemcode, ''::CHARACTER VARYING))::TEXT || (' : '::CHARACTER VARYING)::TEXT) || (COALESCE(COALESCE(ia.itemname, ''::CHARACTER VARYING), (COALESCE(ia2.itemname, ''::CHARACTER VARYING))::CHARACTER VARYING(65535)))::TEXT),
