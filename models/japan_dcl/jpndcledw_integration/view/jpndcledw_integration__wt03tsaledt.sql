...
with tm72del_nm as
(
    select * from {{ source('jpdcledw_integration', 'tm72del_nm') }}
)
,

tm71cancel_nm as
(
    select * from {{ ref('jpndcledw_integration__tm71cancel_nm') }}
)
,
cim01kokya as
(
...
as
(
    select * from {{ source('jpdcledw_integration', 'tm40shihanki') }}
)
,
zaiko_shohin_attr
as
...
as
(
   select * from {{ ref('jpndcledw_integration__tm73nyuhenkan_nm') }}
)
,
tm67juch_nm
as
...
as
(
    select * from {{ ref('jpndcledw_integration__tm66torikei_nm') }}
)
,
tm64kessai_nm
as
...
as
(
    select * from {{ ref('jpndcledw_integration__tm45cardcorp_nm') }}
)
,
cit81salem
as
...
as
(
    select * from {{ ref('jpndcledw_integration__cim43bshobun') }}
)
,
cim42bdaibun
as
(
    select * from {{ ref('jpndcledw_integration__cim42bdaibun') }}
)
,
cim41baitai
as
(
    select * from {{ ref('jpndcledw_integration__cim41baitai') }}
)
,
cim24itbun
as
