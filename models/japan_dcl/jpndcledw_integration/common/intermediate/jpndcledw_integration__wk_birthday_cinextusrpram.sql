...
AS (
    SELECT *
    FROM JPDCLITG_INTEGRATION.c_tbEcShippingResults
    ),
c_tbEcRankAddAmountAdm
AS (
    SELECT *
    FROM JPDCLITG_INTEGRATION.c_tbEcRankAddAmountAdm
    ),
A
AS (
    SELECT memberinfo.diusrid,
        ...
        orderinfo.dirouteid::VARCHAR AS dirouteid --,TO_CHAR(orderinfo.dirouteid) AS dirouteid
        ,
