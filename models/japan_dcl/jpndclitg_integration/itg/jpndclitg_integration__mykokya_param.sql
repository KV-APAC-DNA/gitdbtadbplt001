...
with mykokya_param as (
    select * from DEV_DNA_LOAD.SNAPJPDCLSDL_RAW.MYKOKYA_PARAM
),
...
(
    ...
    TO_CHAR(CONVERT_TIMEZONE('UTC', 'Asia/Tokyo', CAST(CURRENT_TIMESTAMP() AS TIMESTAMP_NTZ(9))), 'MM-DD-YYYY')::varchar(10) as upload_dt,
    TO_CHAR(CONVERT_TIMEZONE('UTC', 'Asia/Tokyo', CAST(CURRENT_TIMESTAMP() AS TIMESTAMP_NTZ(9))), 'hh24:mi:ss')::varchar(8) as upload_time
    from
    transformed
)
select * from final
