{{
    config(
        ...
        pre_hook = "{% if is_incremental() %}
            delete from {{this}} where c_diprivilegeid in (select c_diprivilegeid from {{ source('jpdclsdl_raw', 'c_tbecprivilegekesai') }}) and
            c_dikesaiid	in (select c_dikesaiid from {{ source('jpdclsdl_raw', 'c_tbecprivilegekesai') }});
                    {% endif %}"
    )
}}

with source as(
    select * from {{ source('jpdclsdl_raw', 'c_tbecprivilegekesai') }}
),
final as(
    select 
