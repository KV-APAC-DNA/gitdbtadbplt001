{{
    config(
        ...
        pre_hook = "{% if is_incremental() %}
            delete from {{this}} where c_diprivilegeid in (select c_diprivilegeid from {{ source('jpndclsdl_raw', 'c_tbecprivilegeinquirekesai') }}) and 	
            c_dikesaiid in (select c_dikesaiid from {{ source('jpndclsdl_raw', 'c_tbecprivilegeinquirekesai') }}) and
            diinquireid in (select diinquireid from {{ source('jpndclsdl_raw', 'c_tbecprivilegeinquirekesai') }}) and	
            c_diinquirekesaiid in (select c_diinquirekesaiid from {{ source('jpndclsdl_raw', 'c_tbecprivilegeinquirekesai') }});
                    {% endif %}"
    )
}}

with source as(
    select * from {{ source('jpndclsdl_raw', 'c_tbecprivilegeinquirekesai') }}
),
final as(
    select 
