{{
    config(
        ...
        pre_hook = "{% if is_incremental() %}
                delete from {{this}} where c_diaddadmid in (select c_diaddadmid from {{ source('jpndclsdl_raw', 'c_tbecrankaddamountadm') }}) ;
                    {% endif %}"
    )
}}

with source as(
    select * from {{ source('jpndclsdl_raw', 'c_tbecrankaddamountadm') }}
),
final as(
    select 
