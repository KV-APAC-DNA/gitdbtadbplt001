{{
    config(
        ...
        pre_hook = "{% if is_incremental() %}
                delete from {{this}} where c_dipointissueid in (select c_dipointissueid from {{ source('jpndclsdl_raw', 'c_tbecpointadm') }});
                    {% endif %}"
    )
}}

with source as(
    select * from {{ source('jpndclsdl_raw', 'c_tbecpointadm') }}
),
final as(
    select 
