{{
    config(
        ...
        pre_hook = "{% if is_incremental() %}
            delete from {{this}} where c_dsregularoperatehistid in (select c_dsregularoperatehistid from {{ source('jpndclsdl_raw', 'c_tbecregularoperatehist') }});
                    {% endif %}"
    )
}}

with source as(
    select * from {{ source('jpndclsdl_raw', 'c_tbecregularoperatehist') }}
),
final as(
    select 
