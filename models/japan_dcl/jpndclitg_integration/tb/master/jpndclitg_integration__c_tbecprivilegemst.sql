{{
    config(
        ...
        pre_hook = "{% if is_incremental() %}
                    delete from {{this}} where c_diprivilegeid in (select c_diprivilegeid from {{ source('jpndclsdl_raw', 'c_tbecprivilegemst') }});
                    {% endif %}"
    )
}}

with source as(
    select * from {{ source('jpndclsdl_raw', 'c_tbecprivilegemst') }}
),
final as(
    select 
