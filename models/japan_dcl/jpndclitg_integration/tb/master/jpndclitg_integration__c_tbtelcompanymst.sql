{{
    config(
        ...
        pre_hook = "{% if is_incremental() %}
                delete from {{this}} where c_dstelcompanycd in (select c_dstelcompanycd from {{ source('jpndclsdl_raw', 'c_tbtelcompanymst') }});
                    {% endif %}"
    )
}}

with source as(
    select * from {{ source('jpndclsdl_raw', 'c_tbtelcompanymst') }}
),
final as(
    select 
