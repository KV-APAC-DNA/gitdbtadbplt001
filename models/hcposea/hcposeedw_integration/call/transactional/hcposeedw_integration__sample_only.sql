with itg_call as (
select * from DEV_DNA_CORE.SNAPOSEITG_INTEGRATION.ITG_CALL
),
itg_call_detail as (
select * from DEV_DNA_CORE.SNAPOSEITG_INTEGRATION.ITG_CALL_DETAIL
),
itg_call_discussion as (
select * from DEV_DNA_CORE.OSEITG_INTEGRATION.itg_call_discussion
) ,
itg_recordtype as (
select * from DEV_DNA_CORE.SNAPOSEITG_INTEGRATION.ITG_RECORDTYPE
),
cd as (select * from itg_call_discussion  where nvl(is_deleted,'0') = '0' ),
dim_country as (
select * from DEV_DNA_CORE.SNAPOSEEDW_INTEGRATION.DIM_COUNTRY
),
d as (select * from itg_call_detail d where nvl(d.is_deleted,'0') = '0'  ) ,
dim_employee as (
select * from DEV_DNA_CORE.SNAPOSEEDW_INTEGRATION.DIM_EMPLOYEE
),
dim_profile as (
select * from DEV_DNA_CORE.SNAPOSEEDW_INTEGRATION.DIM_PROFILE
),
dim_organization as (
select * from DEV_DNA_CORE.SNAPOSEEDW_INTEGRATION.DIM_ORGANIZATION
),
dim_date as (
select * from DEV_DNA_CORE.SNAPOSEEDW_INTEGRATION.DIM_DATE
),
...
dim_remote_meeting as (
select * from DEV_DNA_CORE.SNAPOSEEDW_INTEGRATION.DIM_REMOTE_MEETING
),
dim_hcp as (
select * from DEV_DNA_CORE.SNAPOSEEDW_INTEGRATION.DIM_HCP
),
dim_hco as (
select * from DEV_DNA_CORE.SNAPOSEEDW_INTEGRATION.DIM_HCO
),
us_pr as  ( select employee_key,employee_profile_id ,employee_source_id, 
...
SELECT distinct c1.country_code
  ...
  , case 
      when c1.country_code = 'PH' then convert_timezone('PHT',C1.CALL_DATE_TIME) 
       when c1.country_code = 'SG' then convert_timezone ('SGT',C1.CALL_DATE_TIME)
       when c1.country_code = 'ID' then convert_timezone ('UTC+07',C1.CALL_DATE_TIME) 
       when c1.country_code = 'MY' then convert_timezone('MYT',C1.CALL_DATE_TIME) 
       when c1.country_code = 'TH' then convert_timezone ('ICT',C1.CALL_DATE_TIME)
       when c1.country_code = 'VN' then convert_timezone ('ICT',C1.CALL_DATE_TIME) 
       when c1.country_code = 'NP' then convert_timezone ('NPT',C1.CALL_DATE_TIME) 
     end CALL_DATE_TIME
  , case 
       when c1.country_code = 'PH' then convert_timezone('PHT',C1.LAST_MODIFIED_DATE) 
       when c1.country_code ='SG' then convert_timezone ('SGT', C1.LAST_MODIFIED_DATE)
       when c1.country_code ='ID' then convert_timezone ('UTC+07', C1.LAST_MODIFIED_DATE) 
       when c1.country_code = 'MY' then convert_timezone('MYT',C1.LAST_MODIFIED_DATE) 
       when c1.country_code ='TH' then convert_timezone ('ICT', C1.LAST_MODIFIED_DATE)
       when c1.country_code ='VN' then convert_timezone ('ICT', C1.LAST_MODIFIED_DATE) 
       when c1.country_code ='NP' then convert_timezone ('NPT', C1.LAST_MODIFIED_DATE) 
     end LAST_MODIFIED_DATE  
  ,case 
       when c1.country_code = 'PH' then convert_timezone('PHT',C1.MOBILE_LAST_MODIFIED_DATE_TIME) 
       when c1.country_code ='SG' then convert_timezone ('SGT', C1.MOBILE_LAST_MODIFIED_DATE_TIME)
       when c1.country_code ='ID' then convert_timezone ('UTC+07', C1.MOBILE_LAST_MODIFIED_DATE_TIME) 
       when c1.country_code = 'MY' then convert_timezone('MYT',C1.MOBILE_LAST_MODIFIED_DATE_TIME) 
       when c1.country_code ='TH' then convert_timezone ('ICT', C1.MOBILE_LAST_MODIFIED_DATE_TIME)
       when c1.country_code ='VN' then convert_timezone ('ICT', C1.MOBILE_LAST_MODIFIED_DATE_TIME) 
       when c1.country_code ='NP' then convert_timezone ('NPT', C1.MOBILE_LAST_MODIFIED_DATE_TIME)
