...
AS (
    SELECT *
    FROM DEV_DNA_CORE.nnaras01_WORKSPACE.HCPOSEEDW_INTEGRATION__wrk_fact_call_only
    ),
wrk_fact_group_detail
AS (
    SELECT *
    FROM DEV_DNA_CORE.nnaras01_WORKSPACE.HCPOSEEDW_INTEGRATION__WRK_FACT_GROUP_DETAIL
    ),
wrk_fact_detail_only
AS (
    SELECT *
    FROM DEV_DNA_CORE.nnaras01_WORKSPACE.HCPOSEEDW_INTEGRATION__wrk_fact_detail_only
    ),
Sample_Only
AS (
    SELECT *
    FROM DEV_DNA_CORE.nnaras01_WORKSPACE.HCPOSEEDW_INTEGRATION__Sample_Only
    ),
Detail_with_Sample
AS (
    SELECT *
    FROM DEV_DNA_CORE.nnaras01_WORKSPACE.HCPOSEEDW_INTEGRATION__Detail_with_Sample
    ),
Group_Detail_with_Sample
AS (
    SELECT *
    FROM DEV_DNA_CORE.nnaras01_WORKSPACE.HCPOSEEDW_INTEGRATION__Group_Detail_with_Sample
    ),
T1
AS (
    SELECT country_code,
