...
AS (
    SELECT *
    FROM HCPOSEEDW_INTEGRATION.WRK_FACT_CYCLE_PLAN
    ),
...
AS (
    SELECT COUNTRY_KEY,
        ...
        CASE 
            WHEN TO_DATE(END_DATE_KEY, 'YYYYMMDD') - TO_DATE(START_DATE_KEY, 'YYYYMMDD') <= 7
                THEN 'W' ---- WEEKLY
            WHEN TO_DATE(END_DATE_KEY, 'YYYYMMDD') - TO_DATE(START_DATE_KEY, 'YYYYMMDD') > 7
                AND TO_DATE(END_DATE_KEY, 'YYYYMMDD') - TO_DATE(START_DATE_KEY, 'YYYYMMDD') <= 31
                THEN 'M' -- MONTHLY
            WHEN TO_DATE(END_DATE_KEY, 'YYYYMMDD') - TO_DATE(START_DATE_KEY, 'YYYYMMDD') > 31
                AND TO_DATE(END_DATE_KEY, 'YYYYMMDD') - TO_DATE(START_DATE_KEY, 'YYYYMMDD') <= 92
                THEN 'Q' -- QUARTERLY
            WHEN TO_DATE(END_DATE_KEY, 'YYYYMMDD') - TO_DATE(START_DATE_KEY, 'YYYYMMDD') > 92
                AND TO_DATE(END_DATE_KEY, 'YYYYMMDD') - TO_DATE(START_DATE_KEY, 'YYYYMMDD') <= 182
                THEN 'H' -- HALF YEARLY
            WHEN TO_DATE(END_DATE_KEY, 'YYYYMMDD') - TO_DATE(START_DATE_KEY, 'YYYYMMDD') > 182
                THEN 'Y' -- YEARLY 
            ELSE 'Not Applicable'
        ...
        SYSDATE(),
        SYSDATE()
    FROM hcp_osea_edw.WRK_FACT_CYCLE_PLAN
    ),
T2
AS (
    SELECT TARGET.COUNTRY_KEY,
        ...
        SYSDATE(),
        SYSDATE()
    FROM hcp_osea_edw.WRK_FACT_CYCLE_PLAN TARGET,
        hcp_osea_itg.itg_CYCLE_PLAN_DETAIL DETAIL,
        hcp_osea_edw.DIM_PRODUCT_INDICATION PROD
    WHERE DETAIL.CYCLE_PLAN_TARGET_SOURCE_ID = TARGET.CYCLE_PLAN_TARGET_SOURCE_ID
        AND DETAIL.COUNTRY_CODE = TARGET.COUNTRY_CODE
        AND DETAIL.IS_DELETED = 0
        AND DETAIL.PRODUCT_SOURCE_ID = PROD.PRODUCT_SOURCE_ID(+)
        AND DETAIL.COUNTRY_CODE = PROD.COUNTRY_CODE(+)
    ),
FINAL
AS (
