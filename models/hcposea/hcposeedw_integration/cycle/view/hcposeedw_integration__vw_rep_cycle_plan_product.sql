...
derived_table1 AS (
        SELECT fact.mid_date AS DATE,
            ...
            fct1.accounts_specialty_1 AS specialty,
            hier.sector,
        ...
        FROM ((
            ...
            WHERE ((fact_cycle_plan.cycle_plan_type)::TEXT = ('PRODUCT'::CHARACTER VARYING)::TEXT) ) fact
            LEFT JOIN dim_hcp dim_hcp ON (((fact.hcp_key)::TEXT = (dim_hcp.hcp_key)::TEXT))
            ...
            LEFT JOIN (
                SELECT DISTINCT fact_cycle_plan.accounts_specialty_1,
                    fact_cycle_plan.cycle_plan_source_id,
                    fact_cycle_plan.cycle_plan_target_source_id
                FROM fact_cycle_plan
                WHERE ((fact_cycle_plan.cycle_plan_type)::TEXT = ('TARGET'::CHARACTER VARYING)::TEXT)
                    ...
                    (
                        ((fact.cycle_plan_source_id)::TEXT = (fct1.cycle_plan_source_id)::TEXT)
                        AND ((fact.cycle_plan_target_source_id)::TEXT = (fct1.cycle_plan_target_source_id)::TEXT)
                        )
                    )
            LEFT JOIN (
                SELECT count(fact_cycle_plan.cycle_plan_detail_attainment) AS cyc_count,
                    fact_cycle_plan.employee_key,
                    fact_cycle_plan.country_key,
                    fact_cycle_plan.start_date_key,
                    fact_cycle_plan.end_date_key,
                    fact_cycle_plan.cycle_plan_indicator,
                ...
                WHERE (
                        (
                            (
                                ((fact_cycle_plan.cycle_plan_type)::TEXT = ('PRODUCT'::CHARACTER VARYING)::TEXT)
                                AND (fact_cycle_plan.cycle_plan_detail_attainment IS NOT NULL)
                                )
                            AND ((fact_cycle_plan.status_type)::TEXT = ('Approved'::CHARACTER VARYING)::TEXT)
                            )
                        AND ((fact_cycle_plan.cycle_plan_indicator)::TEXT <> ('Not Applicable'::CHARACTER VARYING)::TEXT)
                        )
                GROUP BY fact_cycle_plan.employee_key,
                    fact_cycle_plan.country_key,
                    fact_cycle_plan.start_date_key,
                    fact_cycle_plan.end_date_key,
                    fact_cycle_plan.cycle_plan_indicator
                    ...
                    (
                        (
                            ((factcnt.employee_key)::TEXT = (fact.employee_key)::TEXT)
                            AND ((factcnt.country_key)::TEXT = (fact.country_key)::TEXT)
                            )
                        AND (factcnt.start_date_key = fact.start_date_key)
                        )
                    )
            )
        ),
