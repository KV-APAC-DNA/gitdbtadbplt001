{{
    config(
        ...
        pre_hook = "{% if is_incremental() %}
                delete from {{this}}
                where (mc_cycle_plan_id) in (select mc_cycle_plan_id
                from {{ source('hcposesdl_raw', 'sdl_hcp_osea_mc_cycle_plan') }} stg_mc_cycle_plan
                where stg_mc_cycle_plan.mc_cycle_plan_id = mc_cycle_plan_id);
                {% endif %}"
    )
}}
