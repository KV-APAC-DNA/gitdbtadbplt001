...
(
    ...
    FROM (SELECT CAST(TARGET.FISC_YR AS numeric(18,0) ) AS FISC_YR,
            ...
            UPPER(COALESCE(ACTUAL.EAN, TARGET.EAN)) AS PRODUCT_CODE,
            UPPER(COALESCE(ACTUAL.SKU_DESCRIPTION, TARGET.SKU_DESCRIPTION)) AS PRODUCT_NAME,
            POP.PROD_HIER_L1,	
            POP.PROD_HIER_L2,	
            POP.PROD_HIER_L3,	
            POP.PROD_HIER_L4,	
...
FROM (SELECT CAST(ACTUAL.YEAR AS numeric(18,0) ) AS YEAR,
			 ...
			 ACTUAL.LIST_PRICE,	
             UPPER(COALESCE(ACTUAL.STORE_TYPE, 'NA')) AS STORE_TYPE,
             UPPER(ACTUAL.STORE_GRADE) AS STORE_GRADE,
             --RETAILER.CLASS_DESC AS STORE_SIZE,
			 ACTUAL.REGION AS REGION,
