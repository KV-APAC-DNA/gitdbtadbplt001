...
FROM (
SELECT TARGET.FISC_YR :: numeric(18,0)  AS FISC_YR,		
             TARGET.FISC_PER :: numeric(18,0) AS FISC_PER,		
			 COALESCE(ACTUAL.CNTRY_NM, 'Korea') AS MARKET,		
			 ...
			 /*TARGET.MASTER_CODE AS PRODUCT_CODE,		
             ...
             POP.PROD_HIER_L8,		
             POP.PROD_HIER_L9,		
             UPPER(COALESCE(ACTUAL.SKU_CODE, TARGET.SKU_CODE)) AS MAPPED_SKU_CD,		
			 ...
			 CASE WHEN  TRIM(NVL (NULLIF(ACTUAL.PKA_PRODUCT_KEY,''),'NA')) NOT IN ('N/A','NA') THEN TRIM(NVL (NULLIF(ACTUAL.PKA_PRODUCT_KEY,''),'NA'))		
		...
		WHERE RANK = 1) CUSTOMER ON LTRIM(COALESCE(ACTUAL.SOLD_TO_CODE, TARGET.SOLD_TO_CODE),'0') = LTRIM(CUSTOMER.SAP_CUST_ID ,'0')		--// 		WHERE RANK = 1) CUSTOMER ON LTRIM(ACTUAL.SOLD_TO_CODE,'0') = LTRIM(CUSTOMER.SAP_CUST_ID (+),'0')
