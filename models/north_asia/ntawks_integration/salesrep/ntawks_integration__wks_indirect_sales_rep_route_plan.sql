{{
    ...
    (
        ...
        pre_hook = "{% if is_incremental() %} 
                    ...
                    delete from {{this}} where period >= (select distinct period from select * from {{ source('ntasdl_raw', 'sdl_hk_wingkeung_indirect_sales_rep_route_plan')}} ) and ctry_cd = 'HK' and dstr_cd = '110256';
                    {% endif %}"
    )
}}
