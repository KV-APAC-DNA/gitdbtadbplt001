...
Mannings as (
    ...
    FROM (
            ...
            WHERE x.ctry_cd = 'TW'
                AND x.src_sys_cd = 'Mannings'
            UNION ALL
            SELECT a.*,
                qp.sold_to_party AS sold_to_party,
                CAST(d.ean AS VARCHAR(100)) AS barcode,
                a.src_sys_cd AS sls_grp,
                COALESCE(d.prod_hier_l4, 'Others') AS mysls_brnd_nm,
                COALESCE(d.prod_hier_l3, 'Others') AS mysls_catg,
                b.jnj_sku_code AS sap_matl_num,
                NULL AS prom_sls_amt,
                NULL AS prom_prc_amt,
                a.str_cd AS str_num --msd.dstr_chnl AS dstr_chnl
            FROM itg_pos a
                LEFT JOIN (
                    select *
                    from itg_mds_hk_pos_product_mapping
                ) b ON a.vend_prod_cd = b.prod_code
                and a.src_sys_cd = b.customer_name
                left JOIN (
                    ...
                    group by ean_num,
                        matl_num,
                        sls_org
                ) msd ON b.jnj_sku_code = LTRIM (msd.matl_num, 0)
                LEFT JOIN (
                    SELECT DISTINCT ean,
                        sap_matl_num,
                        prod_hier_l4,
...
carrefour as (
    ...
    FROM (
            ...
            FROM (
                ...
                And x.vend_prod_cd = e.cust_prod_cd
            WHERE x.ctry_cd = 'TW'
                AND x.src_sys_cd = 'Carrefour 家樂福'
            UNION ALL
            SELECT a.*,
                '#' AS sold_to_party,
                CAST(d.ean AS VARCHAR(100)) AS barcode,
            ...
            FROM itg_pos a
                ...
                AND a.ctry_cd = d.cntry
            WHERE a.ctry_cd = 'HK'
                AND a.src_sys_cd = 'Carrefour 家樂福'
        ) src
        LEFT JOIN (
            SELECT DISTINCT matl_num,
                matl_desc
