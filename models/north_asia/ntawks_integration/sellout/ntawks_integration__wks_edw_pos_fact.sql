...
FROM (
  ...
  FROM (
    SELECT a.*,
      ...
      CASE 
        WHEN a.src_sys_cd LIKE '7-11'
          ...
          OR a.src_sys_cd LIKE 'Watsons%'
          THEN CAST(g.barcd AS VARCHAR(100))
    ...
    LEFT JOIN (
      SELECT CASE 
          WHEN cust_nm = 'ibonMart'
            THEN 'ibonMart'
          WHEN cust_nm = 'EC'
            THEN 'EC'
          WHEN cust_nm = '7-11'
            THEN '7-11'
          WHEN cust_nm = 'Carrefour'
            THEN 'Carrefour 家樂福'
          WHEN cust_nm = 'Cosmed'
            THEN 'Cosmed 康是美'
          WHEN cust_nm = 'PX-Civilian'
            THEN 'PX 全聯'
          WHEN cust_nm = 'A-Mart'
            THEN 'A-Mart 愛買'
          WHEN cust_nm = 'Poya'
            THEN 'Poya 寶雅'
          WHEN cust_nm = 'Watsons'
            THEN 'Watsons 屈臣氏'
          WHEN cust_nm = 'RT-Mart'
            THEN 'RT-Mart 大潤發'
          END AS cust_nm,
        cust_prod_cd,
        MIN(barcd) AS barcd
      FROM itg_pos_cust_prod_cd_ean_map
      GROUP BY cust_prod_cd,
        cust_nm
      ) g ON a.vend_prod_cd = g.cust_prod_cd
  ...
  LEFT JOIN (
    ...
    ) d ON CAST(x.barcode AS VARCHAR(40)) = CAST(d.ean AS VARCHAR(40))
  ...
  LEFT JOIN (
    ...
    ) e ON x.pos_dt BETWEEN e.prom_strt_dt
      AND e.prom_end_dt
    AND x.barcode = e.barcd
    AND x.src_sys_cd = e.cust
    AND x.vend_prod_cd = e.cust_prod_cd
  WHERE x.ctry_cd = 'TW'
  AND x.src_sys_cd in ('7-11','PX 全聯','RT-Mart 大潤發')
    -- AND x.src_sys_cd = '"+context.src_sys_cd+"'
  
  UNION ALL
  
  SELECT a.*,
    '#' AS sold_to_party,
    CAST(d.ean AS VARCHAR(100)) AS barcode,
    a.src_sys_cd AS sls_grp,
    COALESCE(d.prod_hier_l4, 'Others') AS mysls_brnd_nm,
    COALESCE(d.prod_hier_l3, 'Others') AS mysls_catg,
    b.matl AS sap_matl_num,
    NULL AS prom_sls_amt,
    NULL AS prom_prc_amt,
    a.str_cd AS str_num
  --msd.dstr_chnl AS dstr_chnl
  FROM itg_pos a
  LEFT JOIN (
    SELECT *
    FROM itg_pos_cust_prod_to_sap_prod_map
    WHERE customer_code = '100003'
    ) b ON a.vend_prod_cd = b.cust_matl_no
  LEFT JOIN (
    SELECT DISTINCT LTRIM(matl_num, 0) AS matl_num,
      sls_org,
      ean_num,
      max(dstr_chnl) AS dstr_chnl
    FROM edw_material_sales_dim
    WHERE sls_org = '1110'
      AND (
        ean_num IS NOT NULL
        AND ean_num <> ''
        AND ean_num NOT IN ('NA', 'N/A', '1', '0', 'TBA', 'TBC')
        )
    GROUP BY ean_num,
      matl_num,
      sls_org
    ) msd ON b.matl = LTRIM(msd.matl_num, 0)
  LEFT JOIN (
    SELECT DISTINCT ean,
      sap_matl_num,
      prod_hier_l4,
      ...
      cntry
    FROM edw_product_attr_dim
    ) d ON msd.ean_num = d.ean
    AND a.ctry_cd = d.cntry
  WHERE a.ctry_cd = 'HK'
    AND a.src_sys_cd = '"+context.src_sys_cd+"'
  ) src
LEFT JOIN (
...
itg_query_parameters where country_code='TW' and parameter_type='sold_to_party') qp on qp.src_sys_cd=a.src_sys_cd) x
        ...
        LEFT JOIN (select CASE
			  ...
			  And x.vend_prod_cd=e.cust_prod_cd
      WHERE x.ctry_cd = 'TW'
      and x.src_sys_cd in ('EC','Carrefour 家樂福','Cosmed 康是美','Poya 寶雅')
        -- AND   x.src_sys_cd = '"+context.src_sys_cd+"'
      UNION ALL
      SELECT a.*, '#' AS sold_to_party,
             CAST(d.ean AS VARCHAR(100)) AS barcode,
             a.src_sys_cd AS sls_grp,
            COALESCE(d.prod_hier_l4,'Others') AS mysls_brnd_nm,
            COALESCE(d.prod_hier_l3,'Others') AS mysls_catg,
             ...
             --msd.dstr_chnl AS dstr_chnl
      FROM  itg_pos  a
        LEFT JOIN (select * from itg_pos_cust_prod_to_sap_prod_map where customer_code = '100003') b ON a.vend_prod_cd = b.cust_matl_no
        left JOIN (SELECT DISTINCT LTRIM(matl_num,0) AS matl_num, sls_org, ean_num, max(dstr_chnl) as dstr_chnl
                   FROM edw_material_sales_dim
                   WHERE sls_org = '1110'
                   AND   (ean_num IS NOT NULL    AND    ean_num <> ''
                   AND ean_num not in ('NA','N/A','1','0','TBA','TBC'))
                  group by ean_num,matl_num,sls_org) msd ON b.matl = LTRIM (msd.matl_num,0)
        LEFT JOIN (SELECT DISTINCT ean, sap_matl_num,  prod_hier_l4, prod_hier_l3, cntry
                   FROM edw_product_attr_dim) d  ON msd.ean_num = d.ean
              AND a.ctry_cd = d.cntry
      WHERE a.ctry_cd = 'HK'   
      -- AND   a.src_sys_cd =  '"+context.src_sys_cd+"'
      ) src
...
FROM (SELECT x.*,
      ...
      FROM (SELECT a.*,
                   ...
                   CASE
                     ...
                     ELSE c.sls_grp
                   END AS sls_grp
            FROM (SELECT * FROM itg_pos WHERE LENGTH(str_cd) > 0) a
              LEFT JOIN (SELECT DISTINCT sls_grp,
                                cust_store_ref,
                                sold_to_party
                         FROM edw_customer_attr_flat_dim
                     ...
                     ON c.cust_store_ref = a.str_cd
                    AND c.sls_grp = a.src_sys_cd
              LEFT JOIN (SELECT para.parameter_value AS cust_nm,
                                ean_map.cust_prod_cd,
                                MIN(ean_map.barcd) AS barcd
                         FROM itg_pos_cust_prod_cd_ean_map ean_map
						 Left Join (select * from itg_query_parameters
											where country_code = 'TW'
											and parameter_type = 'cust_nm') para
						on ean_map.cust_nm = para.parameter_name						 
                         GROUP BY ean_map.cust_prod_cd,
                                  para.parameter_value) g
                     ON a.vend_prod_cd = g.cust_prod_cd
                    AND a.src_sys_cd = g.cust_nm
					left join (select distinct country_code,parameter_name as src_sys_cd,parameter_value as sold_to_party from 
itg_query_parameters where country_code='TW' and parameter_type='sold_to_party') qp on qp.src_sys_cd=a.src_sys_cd) x
        LEFT JOIN (SELECT DISTINCT ean,
               ...
               ON CAST (x.barcode AS VARCHAR (40)) = CAST (d.ean AS VARCHAR (40))
              AND x.ctry_cd = d.cntry
        LEFT JOIN (SELECT para.parameter_value AS cust,
                          barcd,
                          cust_prod_cd,
                          prom_prc,
                          prom_strt_dt,
				   ...
				   Left Join (select * from itg_query_parameters
											where country_code = 'TW'
											and parameter_type = 'cust_nm') para
					on prc_map.cust = para.parameter_name) e
               ON x.pos_dt BETWEEN e.prom_strt_dt
              AND e.prom_end_dt
              AND x.barcode = e.barcd
              AND x.src_sys_cd = e.cust
              AND x.vend_prod_cd = e.cust_prod_cd
      WHERE x.ctry_cd = 'TW'
      AND   x.src_sys_cd = (SELECT parameter_value
      ...
      --msd.dstr_chnl AS dstr_chnl
        ...
        LEFT JOIN (SELECT *
                   FROM itg_pos_cust_prod_to_sap_prod_map
                   WHERE customer_code = '100003') b ON a.vend_prod_cd = b.cust_matl_no
        LEFT JOIN (SELECT DISTINCT LTRIM(matl_num,0) AS matl_num,
                          sls_org,
                          ean_num,
                          MAX(dstr_chnl) AS dstr_chnl
                   FROM edw_material_sales_dim
                   WHERE sls_org = '1110'
                   AND   (ean_num IS NOT NULL AND ean_num <> '' AND ean_num NOT IN ('NA','N/A','1','0','TBA','TBC'))
                   GROUP BY ean_num,
                            matl_num,
                            sls_org) msd ON b.matl = LTRIM (msd.matl_num,0)
        LEFT JOIN (SELECT DISTINCT ean,
                          sap_matl_num,
                          prod_hier_l4,
                          prod_hier_l3,
                          cntry
                   FROM edw_product_attr_dim) d
               ON msd.ean_num = d.ean
              AND a.ctry_cd = d.cntry
      WHERE a.ctry_cd = 'HK'
      AND   a.src_sys_cd = '') src
