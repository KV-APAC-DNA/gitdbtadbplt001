...
select * from {{ ref('ntawks_integration__wks_taiwan_regional_sellout_base') }}
),
...
(SELECT  
	   ...
	   CAST('TWD' AS VARCHAR) AS FROM_CURRENCY,
	   'USD' AS TO_CURRENCY,
...
LEFT JOIN (SELECT DISTINCT 
	...
	WKS_TAIWAN_REGIONAL_SELLOUT_BASE sls_base
	...
	left join (
		...
		ON a.ctry_nm = 'Taiwan' and b.ctry_nm = 'Taiwan' and a.ean_upc = b.ean_upc and a.sku = b.sku
		AND b.latest_nts_date = a.nts_date AND b.row_number = 1
	) pka on ltrim(sls_base.ean, '0') = pka.ean_upc
	left join edw_gch_producthierarchy EGPH on nvl(ltrim(pka.sku, '0'), ltrim(sls_base.matl_num, '0')) = ltrim(EGPH.materialnumber, '0') 
 ...
 (SELECT a.ctry_nm, a.ean_upc, a.sku, a.pka_productkey, a.pka_productdesc
	...
	AND b.latest_nts_date = a.nts_date AND b.row_number = 1 and a.ctry_nm = 'Taiwan') PROD_KEY1
...
LEFT JOIN
 (SELECT a.ctry_nm, a.ean_upc, a.sku, a.pka_productkey, a.pka_productdesc
	...
	ON a.ctry_nm = b.ctry_nm AND a.ean_upc = b.ean_upc AND a.sku = b.sku
	AND b.latest_nts_date = a.nts_date AND b.row_number = 1 and a.ctry_nm = 'Taiwan') PROD_KEY2
...
global_product_franchise::varchar(100) as global_product_franchise,
