...
SELECT inv.*,
    ...
    datediff (
        ...
        last_day(
            to_date(
                left(cal_mnth_id, 4) || right(cal_mnth_id, 2),
                'yyyymm'
            )
        )
    ) diff_weeks,
...
FROM wks_kr_inventory_health_analysis_propagation inv
    LEFT JOIN (
        select *
        from wks_korea_sellout_for_inv_analysis
    ...
    LEFT JOIN (
        select *
        from wks_kr_inventory_healthy_unhealthy_analysis
    ) healthy_inv ON cal_mnth_id = healthy_inv.month
