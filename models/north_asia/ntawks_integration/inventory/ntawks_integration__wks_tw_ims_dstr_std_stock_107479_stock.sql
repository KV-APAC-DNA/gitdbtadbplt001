with source as(
    select * from {{ source('ntasdl_raw', 'sdl_tw_ims_dstr_std_stock_107479_stock') }}
     where filename not in (
        select distinct file_name from {{ source('ntawks_integration', 'TRATBL_sdl_tw_ims_dstr_std_stock_107479_stock__null_test') }}
        union all
