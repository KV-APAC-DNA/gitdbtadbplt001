 ...
 EMD.matl_num AS SAP_MATL_NUM,
        ...
        --  EMD.SAP_PUT_UP_CD AS SAP_PUT_UP_CD,
		 ...
		 -- EMD.greenlight_sku_flag as greenlight_sku_flag,
          ...
          FROM 
         --(Select * from  edw_vw_greenlight_skus where  sls_org in ('3200','320A','320S','321A') ) EMD
...
,      EDW_GCH_PRODUCTHIERARCHY EGPH
      ...
      IN  (SELECT DISTINCT LTRIM(MATL_NUM,'0') FROM edw_material_sales_dim WHERE sls_org in ('3200','320A','320S','321A'))
      )
      where rnk=1
      ),
CUSTOMER AS
      ...
      (
        ...
        UNION ALL
                  ...
                  inv.dstr_cd as dstr_cd,
						...
						case when inv.month is null then null else cast((left(inv.month,4)||'-'||substring(inv.month,5,6)||'-01') as date) end as bill_dt,
						--inv.ean,
                   so_qty as so_qty,
...
Select subj_area,dstr_cd,t1.matl_num,bill_dt,
        CASE WHEN upper(subj_area)='OTC' THEN T1.so_value
        ELSE T1.SO_QTY*nvl(t5.amount,t6.amount) END AS SO_TRD_SLS
