...
with wks_hk_rpt_retail_excellence_gcph as 
(
    select * from {{ ref('ntawks_integration__wks_hk_rpt_retail_excellence_gcph' )}}
),
wks_hk_re_target_compliance as 
(
    select * from {{ ref('ntawks_integration__wks_hk_re_target_compliance' )}}
),

--final cte

edw_hk_rpt_retail_excellence as 
(
    SELECT FISC_YR,
       GCPH.FISC_PER,
       "cluster",
...
FROM WKS_HK_RPT_RETAIL_EXCELLENCE_GCPH GCPH
    LEFT JOIN wks_hk_re_target_compliance TRGT_CMP on (GCPH.fisc_per=TRGT_CMP.fisc_per and UPPER(GCPH.global_product_brand)=UPPER(TRGT_CMP.global_product_brand))
),
...
(
    ...
    from edw_hk_rpt_retail_excellence
)
