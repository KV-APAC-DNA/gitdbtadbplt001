...
FROM (
        SELECT CASE
            ...
            CASE
                WHEN (src4.fisc_wk_num IS NULL) THEN (
                    SELECT y.fisc_wk_num
                    FROM (
        ...
        FROM (
                (
                    (
                        ...
                        LEFT JOIN (
                            ...
                            WHERE (a.wkday = 5)
                            ORDER BY a.cal_day
                        ) src2 ON ((src1.cal_day = src2.cal_day))
                    )
                    LEFT JOIN (
                        SELECT row_number() OVER(
                                PARTITION BY a.fisc_per
                        ...
                        ORDER BY a.cal_wk
                    ) src4 ON ((src1.cal_day = src4.cal_day))
                )
                LEFT JOIN (
                ...
                ) src5 ON ((src1.fisc_per = src5.fisc_per))
            )
