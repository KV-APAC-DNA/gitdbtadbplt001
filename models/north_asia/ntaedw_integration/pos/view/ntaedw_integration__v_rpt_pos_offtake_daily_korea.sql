...
pos as (
    SELECT edw_pos_fact.pos_dt,
        ...
        CASE
            ...
            ) THEN edw_pos_fact.invnt_amt
            ELSE (
                ((edw_pos_fact.invnt_qty)::numeric)::numeric(18, 0) * edw_pos_fact.prom_prc_amt
            )
        END AS invnt_amt,
            ...
            (
                (
                    (
                        ...
                        AND (
                            (edw_customer_attr_flat_dim.sold_to_party)::text <> (''::character varying)::text
                        )
                    )
                    AND (
                        edw_customer_attr_flat_dim.sls_grp_cd IS NOT NULL
                    )
                )
                AND (
                    (edw_customer_attr_flat_dim.sls_grp_cd)::text <> (''::character varying)::text
                )
            )
            AND (
                upper((edw_customer_attr_flat_dim.cntry)::text) like ('%KOREA%'::character varying)::text
            )
        )
),
ichd as (
    ...
    FROM (
            SELECT trim(
                ...
                row_number() OVER(
                    ...
                    edw_product_attr_dim.cntry
                    ORDER BY edw_product_attr_dim.lcl_prod_nm,
                        edw_product_attr_dim.prod_hier_l1,
                        edw_product_attr_dim.prod_hier_l2,
                        edw_product_attr_dim.prod_hier_l3,
                        edw_product_attr_dim.prod_hier_l4,
...
final as (
    SELECT a.pos_dt,
        ...
        CASE
            ...
            ELSE f.prod_hier_l9
        END AS prod_hier_l9,
        COALESCE(a.prom_prc_amt, ((0)::numeric)::numeric(18, 0)) AS price,
        CASE
            WHEN (
    ...
    FROM pos a
        LEFT JOIN cust i ON (
            COALESCE(i.sold_to_party, '~'::character varying)
        )::text = (
            COALESCE(a.sold_to_party, '~'::character varying)
        )::text
        LEFT JOIN ichd e ON (
            COALESCE(e.sold_to_party, '~'::character varying)
        )::text = (
            COALESCE(a.sold_to_party, '~'::character varying)
        )::text
