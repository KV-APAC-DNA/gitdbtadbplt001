...
v_intrm_invc_sls_cust_hier as (
    select * from {{ ref('ntaedw_integration__v_intrm_invc_sls_cust_hier') }}
),
v_intrm_copa_sls as (
    ...
    WHERE (
            (v_intrm_copa_sls_cust_hier.acct_hier_shrt_desc)::text = ('NTS'::character varying)::text
        )
),
a as (
...
derived_table1 as (
    SELECT COALESCE(a.ctry_nm, b.ctry_nm) AS ctry_nm,
        ...
        COALESCE(a.acct_hier_shrt_desc, 'NTS'::character varying) AS acct_hier_shrt_desc,
