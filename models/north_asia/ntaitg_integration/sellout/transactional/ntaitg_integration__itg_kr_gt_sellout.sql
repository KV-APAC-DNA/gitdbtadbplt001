...
SELECT TO_DATE(SHG.IMS_TXN_DT||'15','YYYYMMDD') AS IMS_TXN_DT,
       ...
       (trunc(SHG.UNIT_PRICE,0)*CAST(SHG.QTY AS NUMERIC(21,5))) AS SLS_AMT,
...
SELECT TO_DATE(SLA.IMS_TXN_DT||'15','YYYYMMDD') AS IMS_TXN_DT,
       ...
       (trunc(SLA.UNIT_PRICE,0)*CAST(SLA.QTY AS NUMERIC(21,5))) AS SLS_AMT,
...
SELECT TO_DATE(SNG.IMS_TXN_DT, 'YYYY-MM-DD') AS IMS_TXN_DT,
  ...
  (trunc(SNG.UNIT_PRICE,0) * CAST(SNG.SALES_QTY AS NUMERIC(21, 5))) AS SLS_AMT,
...
Daiso as (
    SELECT TO_DATE(SDG.Year||SDG.Month||'15','YYYYMMDD') AS IMS_TXN_DT,
       ...
       (IDP.UNIT_PRICE*CAST(iff(SDG.QTY='',null,SDG.QTY) AS NUMERIC(21,5))) AS SLS_AMT,
       CAST(iff(SDG.QTY='',null,SDG.QTY) AS INTEGER) SLS_QTY,
       LCD.SUB_CUSTOMER_CODE,
