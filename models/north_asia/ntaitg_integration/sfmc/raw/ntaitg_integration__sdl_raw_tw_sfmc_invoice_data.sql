...
with source as(
    ...
    where file_name not in (
            select distinct file_name from {{ source('ntawks_integration', 'TRATBL_sdl_tw_sfmc_invoice_data__duplicate_test__ff') }}
            union all
            select distinct file_name from {{ source('ntawks_integration', 'TRATBL_sdl_tw_sfmc_invoice_data__null_test__ff') }}
			union all
            select distinct file_name from {{ source('ntawks_integration', 'TRATBL_sdl_tw_sfmc_invoice_data__test_lookup__ff') }}
    )
),
final as
(
