...
{{
    config(
        ...
        pre_hook= "delete from {{this}} where trim(review_date)||trim(brand)||trim(coupang_id)||trim(coupang_product_name)||trim(review_score_star)
                        in 
                        (select distinct trim(review_date)||trim(brand)||trim(coupang_sku_id)||trim(coupang_sku_name)||trim(review_score_star)
                        from {{ source('ntasdl_raw', 'sdl_kr_coupang_daily_brand_reviews') }});"
                )
}}
