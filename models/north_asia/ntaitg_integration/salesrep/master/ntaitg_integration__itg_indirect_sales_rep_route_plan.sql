        ...
        pre_hook = "update {{this}} a
                    set updt_dttm = convert_timezone('UTC', current_timestamp())::timestamp_ntz(9),
                        effctv_end_dt = (
                                ...
                                from dev_dna_core.snapntawks_integration.wks_indirect_sales_rep_route_plan
                                )
                    from (
                        select t1.*
                        from (
...
with itg_indirect_sales_rep_route_plan as
(
    select * from dev_dna_core.snapntaitg_integration.itg_indirect_sales_rep_route_plan
),

wks_indirect_sales_rep_route_plan as
(
    select * from dev_dna_core.snapntawks_integration.wks_indirect_sales_rep_route_plan
),
...
(
    SELECT t2.ctry_cd,
      ...
      (
        SELECT *
        FROM itg_indirect_sales_rep_route_plan
        WHERE effctv_end_dt = to_date('9999-12-31', 'YYYY-MM-DD')
      ) t1,
      (
            SELECT *
