{{
    config(
        ...
        pre_hook = "{% if is_incremental() %}
					...
					from {{ source('ntasdl_raw', 'sdl_kr_coupang_product_ranking_weekly') }} where file_name  not in (
...
with source as (
    ...
    trim(ranking),trim(data_granularity) order by file_name desc) rnk from 
