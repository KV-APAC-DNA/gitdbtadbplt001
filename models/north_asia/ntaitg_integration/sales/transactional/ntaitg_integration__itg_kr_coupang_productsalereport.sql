...
{{
    config(
        ...
        pre_hook= "delete from {{this}}
                    where trim(transaction_date)||trim(sku_id)||trim(barcode)||trim(sku_people)   in 
                    (select distinct trim(transaction_date)||trim(sku_id)||trim(barcode)||trim(sku_people)
                    from {{ source('ntasdl_raw', 'sdl_kr_coupang_productsalereport') }});"         
                )
}}
