...
t2 as (SELECT * FROM CURRENCY WHERE TO_CCY='USD' AND JJ_MNTH_ID=(SELECT MAX(JJ_MNTH_ID) FROM CURRENCY)),
CUSTOMER AS
(
SELECT DISTINCT 
...
t4 as (SELECT * FROM CUSTOMER WHERE RANK=1),
ONSESEA AS
(
--HK --
    SELECT CAL.CAL_YEAR,
...
AND   cal.CAL_YEAR >= (DATE_PART(YEAR,SYSDATE()) -2)  
    GROUP BY
        CAL.CAL_YEAR,
        CAL.CAL_QRTR_NO,
        CAL.cal_MNTH_ID,
