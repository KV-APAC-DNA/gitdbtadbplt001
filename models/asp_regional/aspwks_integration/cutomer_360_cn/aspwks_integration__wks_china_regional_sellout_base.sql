...
(
    select * from DEV_CHNDNA_CORE.CHNEDW_INTEGRATION.V_RPT_JDES_CUBE
),
v_rpt_pos_sales_new as
(
    select * from DEV_DNA_CORE.ALAKSH01_WORKSPACE.V_RPT_POS_SALES_NEW
),
edw_cube_jdesii_jnj_cal_weekly_dim as
(
    select * from alaksh01_workspace.edw_cube_jdesii_jnj_cal_weekly_dim
),
...
(
    ...
    SELECT 
        ...
        nvl(matsls_ean,prmry_upc_cd) as EAN,
        ...
        nvl(matsls_ean,prmry_upc_cd) as msl_product_code,
        sku_name as msl_product_desc,
    ...
    left join (select matl_num,ean_num as matsls_ean
                from 
                (			
                            Select ltrim(matl_num,'0') as matl_num,ltrim(ean_num,'0') as ean_num, 
                            row_number() over (partition by ltrim(matl_num,'0')  order by crt_dttm desc) as rn
    ...
    and (ean_num != 'N/A' and nullif(ean_num,'') is not null)) where rn = 1) matsls on LTRIM(a.vsku_code,'0') = LTRIM(matsls.matl_num,'0')
    ...
    SELECT 
        ...
        coalesce(upc,matsls_ean,prmry_upc_cd),
        ...
        coalesce(upc,matsls_ean,prmry_upc_cd) as MSL_PRODUCT_CODE,
        item as msl_product_desc,
    ...
    where sls_org_co_cd  in (select distinct co_cd from edw_company_dim where crncy_key = 'RMB' and ctry_key = 'CN' 
    and ctry_group = 'China Personal Care'))
    and (ean_num != 'N/A' and nullif(ean_num,'') is not null)) where rn = 1)matsls on LTRIM(a.p_code,'0') = LTRIM(matsls.matl_num,'0')
    left join (select * from (SELECT DISTINCT prmry_upc_cd,LTRIM(MATL_NUM,'0') as MATL_NUM ,row_number() over
    (PARTITION BY prmry_upc_cd order by  crt_dttm desc) as RN FROM edw_material_dim where (nullif(prmry_upc_cd,'') is not null and prmry_upc_cd != 'N/A')
    )
