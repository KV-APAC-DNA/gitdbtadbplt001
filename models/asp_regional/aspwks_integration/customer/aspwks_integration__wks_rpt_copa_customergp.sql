...
(
    ...
    FROM (SELECT copa.fisc_yr,
                ...
                nvl(company.ctry_group,'') as ctry_group,
                nvl(company."cluster",'') as "cluster",
                copa.acct_num,
                copa.prft_ctr,
                copa.obj_crncy_co_obj,
                LTRIM(nvl(copa.matl_num,'')::TEXT,'0'::CHARACTER VARYING::TEXT)::CHARACTER VARYING AS matl_num,
                copa.co_cd,
                nvl(copa.sls_org,'') as sls_org,
                nvl(copa.dstr_chnl,'') as dstr_chnl,
                nvl(copa.div,'') as div,
                LTRIM(nvl(copa.cust_num,'')::TEXT,'0'::CHARACTER VARYING::TEXT)::CHARACTER VARYING AS cust_num,
                exch_rate.from_crncy,
                exch_rate.to_crncy,
                nvl(copa.acct_hier_shrt_desc,'') as acct_hier_shrt_desc,
                CASE
        ...
        GROUP BY nvl(company.ctry_group,''),
                ...
                copa.acct_num,
                copa.prft_ctr,
                LTRIM(nvl(copa.matl_num,'')::TEXT,'0'::CHARACTER VARYING::TEXT),
                copa.co_cd,
                nvl(copa.sls_org,''),
                nvl(copa.dstr_chnl,''),
                nvl(copa.div,''),
                LTRIM(nvl(copa.cust_num,'')::TEXT,'0'::CHARACTER VARYING::TEXT),
                nvl(copa.acct_hier_shrt_desc,''),
                exch_rate.from_crncy,
    ...
    CASE WHEN co_cd in ('3820','4880','8266','4330','7470','7472','9761','747A','7465','9760','8361','836A') and ltrim(fact.acct_num,'0') = '402041' then '0' else 
