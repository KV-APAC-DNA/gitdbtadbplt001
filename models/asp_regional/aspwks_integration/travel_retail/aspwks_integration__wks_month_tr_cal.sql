...
select distinct cal_mo_1 
                 from edw_calendar_dim where cal_mo_1 >= (select min(year_month) from dev_dna_core.snapaspedw_integration.edw_rg_travel_retail)
                  and cal_mo_1 <=  (select to_char(add_months (to_date(max(year_month),'YYYYMM'),1),'YYYYMM') from dev_dna_core.snapaspedw_integration.edw_rg_travel_retail where identifier<>'TARGET') 
                order by 1 
