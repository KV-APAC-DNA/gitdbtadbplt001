      ...
      From
        ( 
          select country, kpi, to_char(scheduleddate,'YYYYMM') as ym
                ...
                , 0 as  MSL_COMPLAINCE_DENOMINATOR_WT 
                , sum(OSA_COMPLAINCE_NUMERATOR ) as OSA_COMPLAINCE_NUMERATOR
                , sum(OSA_COMPLAINCE_DENOMINATOR) as OSA_COMPLAINCE_DENOMINATOR
                , sum(OSA_COMPLAINCE_DENOMINATOR_WT) as OSA_COMPLAINCE_DENOMINATOR_WT
          ...
          from (
                select case when country='China' then 'China Selfcare' else country end as country, kpi, scheduleddate ,latestdate
                      , round((ref_value - actual_value) * weight_oos,4) as OSA_COMPLAINCE_NUMERATOR
                      , ref_value as OSA_COMPLAINCE_DENOMINATOR 
                      , ref_value * round( weight_oos,4) as OSA_COMPLAINCE_DENOMINATOR_WT  
