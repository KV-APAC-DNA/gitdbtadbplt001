...
(
    select * from snapjpnwks_integration.wks_rpt_regional_sellout_offtake_npd
),
in_wks_rpt_regional_sellout_offtake_npd as
(
    select * from indwks_integration.wks_rpt_regional_sellout_offtake_npd
    --{{ ref('indwks_integration__wks_rpt_regional_sellout_offtake_npd') }}
),
sdl_raw_sap_bw_price_list as
(
    select * from DEV_DNA_LOAD.SNAPASPSDL_RAW.SDL_RAW_SAP_BW_PRICE_LIST
),
...
(
    select * from snaposewks_integration.wks_singapore_regional_sellout_offtake_npd
    --{{ ref('sgpwks_integration__wks_singapore_regional_sellout_offtake_npd') }}
),
...
(
    select * from snaposewks_integration.wks_rpt_regional_sellout_offtake_npd
    --{{ ref('thawks_integration__wks_rpt_regional_sellout_offtake_npd') }}
),
wks_rpt_regional_my_sellout_offtake_npd as
(
    select * from snaposewks_integration.wks_rpt_regional_my_sellout_offtake_npd
    --{{ ref('myswks_integration__wks_rpt_regional_my_sellout_offtake_npd') }}
),
wks_korea_regional_sellout_offtake_npd as
(
    select * from snapntawks_integration.wks_korea_regional_sellout_offtake_npd
    --{{ ref('ntawks_integration__wks_korea_regional_sellout_offtake_npd') }}
),
wks_rpt_regional_id_sellout_offtake_npd as
(
    select * from idnwks_integration.wks_rpt_regional_id_sellout_offtake_npd
    --{{ ref('idnwks_integration__wks_rpt_regional_id_sellout_offtake_npd') }}
),
wks_hong_kong_regional_sellout_offtake_npd as
(
    select * from DEV_DNA_CORE.NNARAS01_WORKSPACE.NTAWKS_INTEGRATION__WKS_HONG_KONG_REGIONAL_SELLOUT_OFFTAKE_NPD
    --{{ ref('ntawks_integration__wks_hong_kong_regional_sellout_offtake_npd') }}
),
wks_taiwan_regional_sellout_offtake_npd as 
(
    select * from snapntawks_integration.wks_taiwan_regional_sellout_offtake_npd
    --{{ ref('ntawks_integration__wks_taiwan_regional_sellout_offtake_npd') }}
),
wks_vietnam_regional_sellout_offtake_npd as
(
    select * from vnmwks_integration.wks_vietnam_regional_sellout_offtake_npd
    --{{ ref('vnwks_integration__wks_vietnam_regional_sellout_offtake_npd') }}
),
final as 
(
    ...
    SELECT * FROM (SELECT main.year::integer as year,
        ...
        CASE WHEN FIRST_SCAN_FLAG_PARENT_CUSTOMER_LEVEL_INITIAL='Y' AND cal_date<=dateadd(week,(select param_value from itg_mds_ap_customer360_config where code='npd_flag_period_weeks')::integer,Customer_Product_Min_Date) THEN 'Y' ELSE 'N' END AS NPD_FLAG_PARENT_CUSTOMER_LEVEL ,
        main.Customer_Product_Desc,
        null as msl_product_code, --ltrim(main.msl_product_code,'0') as msl_product_code,
        null as msl_product_desc, --main.msl_product_desc,
        'NA' AS store_grade,
        null as retail_env, --main.retail_env,
        null as channel, --main.channel,
        main.crtd_dttm,
        main.updt_dttm
