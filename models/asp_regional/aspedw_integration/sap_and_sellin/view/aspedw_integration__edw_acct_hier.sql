...
edw_account_jjplbw_hier as (
    ...
    where
        (length(cast((hier_node) as text)) != 0)
),
...
acct_hier_dim_transformed as (
    ...
    inner join edw_account_xref as b
        on (
            (upper(cast((a.acct_col) as text))= upper(cast((b.lookup_col_name) as text)))
            and 
            (cast((a.bravo_acct_no) as text) = cast((b.lookup_value) as text))
        )
),
