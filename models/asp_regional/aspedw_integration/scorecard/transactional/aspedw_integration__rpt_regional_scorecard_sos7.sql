...
Select 'PERFECTSTORE' as Datasource
      ...
      From
      (            
          select country, 'SOS COMPLAINCE' as kpi, to_char(scheduleddate,'yyyymm') ym, latestdate,
                          case when sum(num_value) > sum(den_value)  then
                                                  sum(den_value *   weight_sos  ) 
                              else 
                                                  sum(num_value *   weight_sos )
                              end  as SOS_COMPLAINCE_NUMERATOR,
                              sum(den_value) as SOS_COMPLAINCE_DENOMINATOR,
                              sum(den_value * weight_sos)  as SOS_COMPLAINCE_DENOMINATOR_WT
                    
          from
          (
              select case when country='China' then 'China Selfcare' else country end as country, customername, scheduleddate, latestdate, segment, category, sum(num_value) num_value, sum(den_value) den_value, min(weight_sos) as weight_sos
              from
               (   select   country,  customername,  scheduleddate , latestdate, prod_hier_l5 as segment,prod_hier_l4 as category, ques_type,
                   ...
                   where  kpi = 'SOS COMPLIANCE' 
                    --and ( ltrim(actual_value) ~ '^[0-9]+\\.[0-9]+$' or  ltrim(actual_value) ~'^[0-9]+$' ) 
