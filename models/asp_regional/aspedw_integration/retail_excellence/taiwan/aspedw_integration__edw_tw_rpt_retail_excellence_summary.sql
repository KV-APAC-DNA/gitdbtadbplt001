...
with tw_edw_rpt_retail_excellence_summary_base as (
    select * from {{ ref('aspedw_integration__edw_tw_rpt_retail_excellence_summary_base') }}
),
...
--Final CTE
tw_edw_rpt_retail_excellence_summary as (
SELECT FISC_YR,
       FISC_PER,
       "cluster",
       MARKET,
...
final as(
    ...
    ,l12m_numeric_distribution :: numeric(38,6) as l12m_numeric_distribution
        from tw_edw_rpt_retail_excellence_summary
)
--Final select
select * from final 