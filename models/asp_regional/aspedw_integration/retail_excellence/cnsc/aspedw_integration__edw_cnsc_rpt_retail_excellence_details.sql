...
--Final CTE
cnsc_edw_rpt_retail_excellence_details as (
SELECT FISC_YR,
      ...
      FROM cnsc_edw_rpt_retail_excellence
	  WHERE  market = 'China Selfcare' and RETAIL_ENVIRONMENT not in (select distinct parameter_value from itg_query_parameters where parameter_name='EXCLUDE_RE_RETAIL_ENV' and country_code='CNSC') 
AND   FISC_PER >= (SELECT REPLACE(SUBSTRING(add_months (TO_DATE(MAX(fisc_per)::varchar,'YYYYMM'),-1),1,7),'-','')::INTEGER 
                   FROM cnsc_edw_rpt_retail_excellence)
AND   FISC_PER <= (SELECT MAX(fisc_per) FROM cnsc_edw_rpt_retail_excellence)
)
,
--Final CTE
final as (
...
from cnsc_edw_rpt_retail_excellence_details
)
--Final select
select * from final