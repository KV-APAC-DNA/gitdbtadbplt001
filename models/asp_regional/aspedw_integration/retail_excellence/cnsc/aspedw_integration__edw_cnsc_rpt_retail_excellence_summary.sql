...
--Import CTE
with cnsc_edw_rpt_retail_excellence_summary_base as (
    select * from {{ ref('aspedw_integration__edw_cnsc_rpt_retail_excellence_summary_base') }}
),
...
--Final CTE
cnsc_edw_rpt_retail_excellence_summary_final as (
SELECT FISC_YR,
       FISC_PER,
       CLUSTER,
       MARKET,
 ...
 WHERE  Market='China Selfcare' AND RETAIL_ENVIRONMENT not in (select distinct parameter_value from itg_query_parameters where parameter_name='EXCLUDE_RE_RETAIL_ENV' and country_code='CNSC') and
	  FISC_PER > TO_CHAR(ADD_MONTHS((SELECT to_date(MAX(fisc_per)::varchar,'YYYYMM') FROM cnsc_edw_rpt_retail_excellence_summary_base),-15),'YYYYMM')
  AND FISC_PER <= (select max(fisc_per) FROM cnsc_edw_rpt_retail_excellence_summary_base)
...
cnsc_edw_rpt_retail_excellence_summary as
(
  select 
  ...
  from cnsc_edw_rpt_retail_excellence_summary_final
)


--Final select
select * from cnsc_edw_rpt_retail_excellence_summary 