...
WHERE FISC_PER >= to_char((to_date(add_months(TO_DATE(TO_VARCHAR(select MAX(fisc_per) from v_edw_sg_rpt_retail_excellence), 'YYYYMM'),-1)))::timestamp without time zone,'YYYYMM'::text)::NUMERIC(18,0)
AND   FISC_PER <= (SELECT MAX(FISC_PER) FROM v_edw_sg_rpt_retail_excellence)
