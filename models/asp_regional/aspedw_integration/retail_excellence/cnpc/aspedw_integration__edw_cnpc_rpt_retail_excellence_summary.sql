...
WHERE  Market='China Personal Care' AND data_src is not null and
    RETAIL_ENVIRONMENT not in (select distinct parameter_value from itg_query_parameters where parameter_name='EXCLUDE_RE_RETAIL_ENV' and country_code='CNPC') and		--//     RETAIL_ENVIRONMENT not in (select distinct parameter_value from rg_itg.itg_query_parameters where parameter_name='EXCLUDE_RE_RETAIL_ENV' and country_code='CNPC') and
	  FISC_PER > TO_CHAR(ADD_MONTHS((SELECT to_date(MAX(fisc_per)::varchar,'YYYYMM') FROM cnpc_edw_rpt_retail_excellence_summary_base),-15),'YYYYMM')		--// 	  FISC_PER > TO_CHAR(ADD_MONTHS((SELECT to_date(MAX(fisc_per),'YYYYMM') FROM CN_EDW.CNPC_EDW_RPT_RETAIL_EXCELLENCE_SUMMARY_BASE),-15),'YYYYMM')
  AND FISC_PER <= (select max(fisc_per) FROM cnpc_edw_rpt_retail_excellence_summary_base)		--//   AND FISC_PER <= (select max(fisc_per) FROM CN_EDW.CNPC_EDW_RPT_RETAIL_EXCELLENCE_SUMMARY_BASE)
