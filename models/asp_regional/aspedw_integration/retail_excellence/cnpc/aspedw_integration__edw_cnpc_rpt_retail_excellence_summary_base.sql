...
with cnpc_edw_rpt_retail_excellence as (
    select * from {{ source('chnedw_integration', 'v_edw_rpt_retail_excellence_cnpc') }}
),
--Logical CTE
...
Select FISC_YR,
      ...
      NUll AS PROD_HIER_L6,
       ...
       TARGET_COMPLAINCE
FROM cnpc_edw_rpt_retail_excellence FLAGS WHERE market='China Personal Care'		--// FROM CN_EDW.EDW_RPT_RETAIL_EXCELLENCE_CNPC FLAGS WHERE market='China Personal Care'
...
--Final select
select * from cnpc_edw_rpt_retail_excellence_summary_base 