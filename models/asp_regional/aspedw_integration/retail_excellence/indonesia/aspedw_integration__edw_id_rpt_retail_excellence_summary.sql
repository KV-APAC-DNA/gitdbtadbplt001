--Import CTE
with id_edw_rpt_retail_excellence_summary_base as (
    select * from {{ ref('aspedw_integration__edw_id_rpt_retail_excellence_summary_base') }}
),
...
SYSDATE()		--// SYSDATE
 ...
 WHERE upper(RETAIL_ENVIRONMENT) not in (select distinct parameter_value from ITG_QUERY_PARAMETERS where parameter_name='EXCLUDE_RE_RETAIL_ENV' and country_code='ID') and		--//  WHERE upper(RETAIL_ENVIRONMENT) not in (select distinct parameter_value from rg_itg.itg_query_parameters where parameter_name='EXCLUDE_RE_RETAIL_ENV' and country_code='ID') and
  ...
  AND FISC_PER <= (select max(fisc_per) FROM id_edw_rpt_retail_excellence_summary_base)		--//   AND FISC_PER <= (select max(fisc_per) FROM ID_EDW.EDW_RPT_ID_RE_SUMMARY_BASE)
 --AND FISC_PER <= (select replace(substring(add_months(to_date(max(fisc_per),'YYYYMM'),-1),1,7),'-','')::integer -- remove -1 in detail & summary
...
--Final select
select * from id_edw_rpt_retail_excellence_summary 