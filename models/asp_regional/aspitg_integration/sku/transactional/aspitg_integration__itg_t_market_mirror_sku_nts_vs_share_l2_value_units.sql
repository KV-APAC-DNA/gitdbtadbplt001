{{
    config(
        ...
        delete from {{this}} where (SELECT COUNT(*) FROM {{ source('aspsdl_raw', 'sdl_t_market_mirror_sku_nts_vs_share_l2_value_units') }}) != 0;
        {% endif %}"
    )
}}
...
with source as 
(
    select * from {{ source('aspsdl_raw', 'sdl_t_market_mirror_sku_nts_vs_share_l2_value_units') }}
),
final as
(
