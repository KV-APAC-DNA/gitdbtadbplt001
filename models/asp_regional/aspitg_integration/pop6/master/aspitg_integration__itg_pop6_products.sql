...
wks as (
                     SELECT prod.cntry_cd,
                            ...
                            prod.effective_from,
                            CASE 
                                   WHEN prod.effective_to IS NULL
                                          THEN dateadd(DAY, - 1, convert_timezone('Asia/Singapore', current_timestamp())::timestamp_ntz(9))
...
SELECT * FROM wks
UNION ALL
SELECT * FROM itg_pop6_products WHERE productdb_id NOT IN (
    SELECT productdb_id
    FROM wks)
),
final as (
