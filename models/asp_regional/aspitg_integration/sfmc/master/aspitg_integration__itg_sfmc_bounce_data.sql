    ...
    config(
        ...
        pre_hook=[
                "
                    ...
                    from {{ source('thasdl_raw', 'sdl_th_sfmc_bounce_data') }} sdl
            ...
            ,"{% if var('sfmc_job_to_execute') == 'th_sfmc_files' %}
                    ...
                    from {{ source('thasdl_raw','sdl_th_sfmc_bounce_data') }}
                    where file_name not in (
                            select distinct file_name from {{ source('thawks_integration', 'TRATBL_sdl_th_sfmc_bounce_data__duplicate_test') }}
                            union all
                            select distinct file_name from {{ source('thawks_integration', 'TRATBL_sdl_th_sfmc_bounce_data__null_test') }}
                    ...
                    delete from {{this}} where event_date >= (select min(event_date) from {{ source('phlsdl_raw','sdl_ph_sfmc_bounce_data') }}) and cntry_cd = 'PH';
