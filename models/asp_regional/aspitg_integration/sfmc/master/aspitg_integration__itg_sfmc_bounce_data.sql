    ...
    config(
        ...
        pre_hook="{%if is_incremental()%}
                {% if var('sfmc_job_to_execute') == 'th_sfmc_files' %}
                    ...
                    delete from {{this}} where event_date >= (select min(event_date) from {{ source('phlsdl_raw','sdl_ph_sfmc_bounce_data') }}
                    where file_name not in (
                    ...
                    delete from {{this}} where event_date >= (select min(event_date) from {{ source('ntasdl_raw', 'sdl_tw_sfmc_bounce_data') }}
                    where file_name not in (
                    select distinct file_name from {{ source('ntawks_integration', 'TRATBL_sdl_tw_sfmc_bounce_data__duplicate_test__ff') }}
                    union all
                    select distinct file_name from {{ source('ntawks_integration', 'TRATBL_sdl_tw_sfmc_bounce_data__null_test__ff') }}
                    union all
                    select distinct file_name from {{ source('ntawks_integration', 'TRATBL_sdl_tw_sfmc_bounce_data__test_lookup__ff') }}
                    )) and cntry_cd = 'TW';
                    {% endif %}
                    {% endif %}"
    )
