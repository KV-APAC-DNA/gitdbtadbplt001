...
{{
    config(
        ...
        delete from {{this}} where event_date >= (select min(event_date) from {{ source('thasdl_raw','sdl_th_sfmc_click_data') }}
        where file_name not in (
            select distinct file_name from {{ source('thawks_integration', 'TRATBL_sdl_th_sfmc_click_data__null_test') }}
            union all
            select distinct file_name from {{ source('thawks_integration', 'TRATBL_sdl_th_sfmc_click_data__duplicate_test') }}
			union all
            select distinct file_name from {{ source('thawks_integration', 'TRATBL_sdl_th_sfmc_click_data__lookup_test') }}
        )) and cntry_cd = 'TH';
        {% elif var('sfmc_job_to_execute') == 'ph_sfmc_files' %}
        delete from {{this}} where event_date >= (select min(event_date) from {{ source('phlsdl_raw','sdl_ph_sfmc_click_data') }}
            where file_name not in (
        ...
        delete from {{this}} where event_date >= (select min(event_date) from {{ source('ntasdl_raw','sdl_tw_sfmc_click_data') }}
        where file_name not in (
            select distinct file_name from {{ source('ntawks_integration', 'TRATBL_sdl_tw_sfmc_click_data__test_null__ff') }}
            union all
            select distinct file_name from {{ source('ntawks_integration', 'TRATBL_sdl_tw_sfmc_click_data__test_duplicate__ff') }}
			union all
            select distinct file_name from {{ source('ntawks_integration', 'TRATBL_sdl_tw_sfmc_click_data__test_lookup__ff') }}
        )) and cntry_cd = 'TW';
        {% endif %}
        {% endif %}
        "
