...
final as(
    select
        ...
        payer as payer,
        plant as plant,
        iff(right(bic_zzp_itm,1)='-',concat('-',replace(bic_zzp_itm,'-','')),bic_zzp_itm) as zzp_itm,
        ...
        iff(right(s_ord_item,1)='-',concat('-',replace(s_ord_item,'-','')),s_ord_item) as s_ord_item,
        doc_type as doc_type,
        iff(right(salesemply,1)='-',concat('-',replace(salesemply,'-','')),salesemply) as salesemply,
        salesorg as salesorg,
        item_categ as item_categ,
        ship_to as ship_to,
        sold_to as sold_to,
        iff(right(bic_zblqtycse,1)='-',concat('-',replace(bic_zblqtycse,'-','')),bic_zblqtycse) as zblqtycse,
