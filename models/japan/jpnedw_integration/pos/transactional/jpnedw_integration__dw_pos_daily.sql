...
aeon as(
    ...
    from wk_pos_daily_aeon
        ...
        where TO_DATE(wk_pos_daily_aeon.upload_dt, 'MM-DD-YYYY') > (select max(TO_DATE(upload_dt, 'MM-DD-YYYY')) {{ this }}) 
        {% endif %}
),
csms as(
    select
    ...
    from wk_pos_daily_csms
        {% if is_incremental() %}
        -- this filter will only be applied on an incremental run
        where TO_DATE(wk_pos_daily_csms.upload_dt, 'MM-DD-YYYY') > (select max(TO_DATE(upload_dt, 'MM-DD-YYYY')) {{ this }}) 
        {% endif %}
),
dnki as(
    select
    ...
    from wk_pos_daily_dnki
        {% if is_incremental() %}
        -- this filter will only be applied on an incremental run
        where TO_DATE(wk_pos_daily_dnki.upload_dt, 'MM-DD-YYYY') > (select max(TO_DATE(upload_dt, 'MM-DD-YYYY')) {{ this }}) 
        {% endif %}
),
otherss as(
    select
    ...
    from wk_pos_daily_others
        {% if is_incremental() %}
        -- this filter will only be applied on an incremental run
        where TO_DATE(wk_pos_daily_others.upload_dt, 'MM-DD-YYYY') > (select max(TO_DATE(upload_dt, 'MM-DD-YYYY')) {{ this }}) 
        {% endif %}
),
tsur as(
    select
    ...
    from wk_pos_daily_tsur
        {% if is_incremental() %}
        -- this filter will only be applied on an incremental run
        where TO_DATE(wk_pos_daily_tsur.upload_dt, 'MM-DD-YYYY') > (select max(TO_DATE(upload_dt, 'MM-DD-YYYY')) {{ this }}) 
        {% endif %}
),
sugi as(
    select
    ...
    from wk_pos_daily_sugi
        {% if is_incremental() %}
        -- this filter will only be applied on an incremental run
        where TO_DATE(wk_pos_daily_sugi.upload_dt, 'MM-DD-YYYY') > (select max(TO_DATE(upload_dt, 'MM-DD-YYYY')) {{ this }}) 
        {% endif %}
),
wlca as(
    select
    ...
    from wk_pos_daily_wlca
        {% if is_incremental() %}
        -- this filter will only be applied on an incremental run
        where TO_DATE(wk_pos_daily_wlca.upload_dt, 'MM-DD-YYYY') > (select max(TO_DATE(upload_dt, 'MM-DD-YYYY')) {{ this }}) 
        {% endif %}
),
transformed as(
    select * from aeon
