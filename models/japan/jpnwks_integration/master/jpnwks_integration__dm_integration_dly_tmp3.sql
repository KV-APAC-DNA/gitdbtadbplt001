{{
    ...
    (
        pre_hook = 
                    "
                    DELETE
                    FROM {{ ref('jpnwks_integration__dm_integration_dly_tmp2') }} ab USING {{ ref('jpnedw_integration__mt_constant_range') }} mt
...
tmp2 AS
(
    select * from {{ ref('jpnwks_integration__dm_integration_dly_tmp2') }}
),
trns
AS (
