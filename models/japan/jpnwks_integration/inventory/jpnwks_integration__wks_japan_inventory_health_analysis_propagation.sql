...
jp_inv_coverage_area_region_mapping as(
    select * from jpnedw_integration.jp_inv_coverage_area_region_mapping
),
edi_cstm_m as(
    select * from jpnedw_integration.edi_cstm_m
),
wks_japan_inventory_health_analysis_propagation_prestep as(
    select * from {{ ref('jpnwks_integration__wks_japan_inventory_health_analysis_propagation_prestep') }}
),
CSTM_M as(
...
Select cntry_key,cntry_nm,rate_type,from_ccy,to_ccy,valid_date,jj_year,jj_mnth_id as MNTH_ID,(cast(EXCH_RATE as numeric(15,5))) as EXCH_RATE
  FROM vw_edw_reg_exch_rate where cntry_key='JP' and jj_mnth_id>= (date_part(year, convert_timezone('UTC', current_timestamp())) -2) and  to_ccy='USD'
...
from wks_japan_inventory_health_analysis_propagation_prestep inv
...
Case 
when ctry_key='JP' then cast((gts_value *exch_rate)/1000 as numeric(38,5))
end as GTS_USD, case when ctry_key='JP' then cast((si_inv_db_val *exch_rate)/1000 as numeric(38,5)) end as  si_inv_db_val_usd
...
,
round(cast(si_sls_qty as numeric(38,5)),5) as si_sls_qty,round(cast(si_gts_val as numeric (38,5)),5) as si_gts_val,
round(cast(si_gts_val_usd as numeric(38,5)),5) as si_gts_val_usd,round(cast (inventory_quantity as numeric(38,5)),5) as inventory_quantity,round(cast(inventory_val as numeric(38,5)),5) as inventory_val, round(cast (inventory_val_usd as numeric(38,5)),5) as inventory_val_usd,round(cast (so_sls_qty as numeric(38,5)),5) as so_sls_qty, round(cast (so_grs_trd_sls as numeric(38,5)),5) as so_grs_trd_sls,
so_grs_trd_sls_usd as so_grs_trd_sls_usd,round(cast (COPA.gts as numeric(38,5)),5)as SI_ALL_DB_VAL, round(cast (COPA.gts_usd as numeric (38,5)),5)
 as SI_ALL_DB_VAL_USD,round(cast (COPA.si_inv_db_val as numeric(38,5)),5) as si_inv_db_val,
