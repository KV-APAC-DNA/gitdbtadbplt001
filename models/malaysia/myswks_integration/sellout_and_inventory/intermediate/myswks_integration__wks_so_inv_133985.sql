with source as
(
    select  * from {{ source('myssdl_raw','sdl_so_inv_133985') }} where file_name not in
            ( 
            select distinct file_name from {{ source('myswks_integration', 'TRATBL_sdl_so_inv_133985__lookup_test') }}
            ) 
),
