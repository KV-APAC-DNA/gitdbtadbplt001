with source as
(
    select  * from {{ source('myssdl_raw','sdl_so_inv_135976') }} where file_name not in
            ( 
            select distinct file_name from {{ source('myswks_integration', 'TRATBL_sdl_so_inv_135976__lookup_test') }}
            ) 
),
