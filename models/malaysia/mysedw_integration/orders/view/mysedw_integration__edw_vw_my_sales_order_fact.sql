...
final as(
    SELECT DISTINCT
        CASE WHEN (CAST((edw_sales_order_fact.sls_org) AS TEXT) = CAST('2100' AS TEXT)) THEN CAST('MY' AS TEXT) ELSE CAST(NULL AS TEXT) END AS cntry_key,
        CASE WHEN (CAST((edw_sales_order_fact.sls_org) AS TEXT) = CAST('2100' AS TEXT)) THEN CAST('MALAYSIA' AS TEXT)ELSE CAST(NULL AS TEXT) END AS cntry_nm,
        edw_sales_order_fact.doc_dt,
        edw_sales_order_fact.bill_dt,
        LTRIM(CAST((edw_sales_order_fact.doc_num) AS TEXT), CAST('0' AS TEXT)) AS sls_doc_num,
        LTRIM(CAST((
