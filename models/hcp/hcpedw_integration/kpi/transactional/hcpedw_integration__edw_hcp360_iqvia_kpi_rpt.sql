...
WHERE upper(SOURCE_SYSTEM) = 'VENTASYS'
...
FROM (SELECT BRAND, ACTIVITY_DATE, HCP_ID, PRESCRIPTION_ID,
             ...
             CALL_DURATION
      FROM EDW_HCP360_KPI_RPT
      WHERE upper(SOURCE_SYSTEM) = 'VENTASYS' AND TRANSACTION_FLAG = 'Y')
GROUP BY 1,
         2,
         3,
         4),
...
FROM EDW_HCP360_KPI_RPT
WHERE upper(SOURCE_SYSTEM) = 'VENTASYS' AND TRANSACTION_FLAG = 'Y' AND CALL_SOURCE_ID IS NOT NULL 
AND CALL_TYPE = 'Phone'
GROUP BY 1,
         2,
         3,
...
FROM EDW_HCP360_KPI_RPT
WHERE upper(SOURCE_SYSTEM) = 'VENTASYS'  AND CALL_TYPE = 'Video' AND TRANSACTION_FLAG = 'Y' AND CALL_SOURCE_ID IS NOT NULL
GROUP BY 1,
         2,
         3,
         4),
...
FROM EDW_HCP360_KPI_RPT
WHERE (upper(SOURCE_SYSTEM) = 'VENTASYS'  AND CALL_TYPE LIKE 'Face to Face%' AND TRANSACTION_FLAG = 'Y' AND CALL_SOURCE_ID IS NOT NULL)
   OR (upper(SOURCE_SYSTEM) = 'VENTASYS'  AND CALL_TYPE IS NULL AND TRANSACTION_FLAG = 'Y' AND CALL_SOURCE_ID IS NOT NULL 
       AND TO_CHAR(ACTIVITY_DATE,'YYYY')= '2022' AND upper(TO_CHAR(ACTIVITY_DATE,'MON')) IN ('APR','MAY'))
GROUP BY 1,
         2,
         3,
         4),
...
FROM (SELECT BRAND,
      ...
      FROM EDW_HCP360_KPI_RPT
      WHERE upper(SOURCE_SYSTEM) = 'VENTASYS' AND CALL_SOURCE_ID IS NOT NULL 
...
(SELECT BRAND,
      ...
      FROM EDW_HCP360_KPI_RPT
      WHERE upper(SOURCE_SYSTEM) =  'VEEVA' 
      GROUP BY 1,
               2,
               3,
               4,5),
...
FROM EDW_HCP360_KPI_RPT
WHERE RTRIM(upper(SOURCE_SYSTEM)) =  'SFMC' AND EMAIL_ACTIVITY_TYPE = 'SENT'
GROUP BY 1,
         2,
         3,
         4,
...
FROM EDW_HCP360_KPI_RPT
WHERE RTRIM(upper(SOURCE_SYSTEM)) =  RTRIM('SFMC') AND EMAIL_ACTIVITY_TYPE = 'SENT'
...
FROM EDW_HCP360_KPI_RPT
WHERE RTRIM(upper(SOURCE_SYSTEM)) =  RTRIM('SFMC') AND EMAIL_ACTIVITY_TYPE = 'OPEN'
AND   IS_UNIQUE = 'Y'
GROUP BY 1,
         2,
         3,
...
FROM EDW_HCP360_KPI_RPT
WHERE RTRIM(upper(SOURCE_SYSTEM)) =  RTRIM('SFMC') AND EMAIL_ACTIVITY_TYPE = 'CLICK'
GROUP BY 1,
         2,
         3,
         4,
...
FROM EDW_HCP360_KPI_RPT
WHERE RTRIM(upper(SOURCE_SYSTEM)) =  RTRIM('SFMC') AND EMAIL_ACTIVITY_TYPE = 'UNSUBSCRIBE'
GROUP BY 1,
         2,
         3,
         4,
...
FROM EDW_HCP360_KPI_RPT
WHERE RTRIM(upper(SOURCE_SYSTEM)) =  RTRIM('SFMC') AND EMAIL_ACTIVITY_TYPE = 'FORWARD'
GROUP BY 1,
         2,
         3,
         4,
...
(SELECT DIM.hcp_id,
 ...
 INNER JOIN (SELECT EMAIL,CONSENT_UPDATE_DATETIME as CONSENT_DATE, ROW_NUMBER() OVER(PARTITION BY nullif(EMAIL,'') ORDER BY cust_entered_date) AS RNK
             FROM   EDW_HCP360_IN_VENTASYS_HCP_DIM_LATEST) PD
         ON nullif(DIM.EMAIL,'') = nullif(PD.EMAIL,'')
        AND PD.RNK =1
...
left join (select email_name,hcp_id from EDW_HCP360_KPI_RPT where upper(source_system) = upper('Ventasys') group by 1,2) vent_hcp
...
(
    ...
    FROM  (SELECT 'IQVIA_DASHBOARD'  as data_source,
        ...
        FROM  edw_hcp360_kpi_rpt
        WHERE upper(SOURCE_SYSTEM) =  'GA360' 
        AND country = 'IN'
...
SELECT 'IQVIA_DASHBOARD'  AS data_source,
    ...
    FROM  EDW_HCP360_KPI_RPT
    WHERE upper(SOURCE_SYSTEM) =  'IQVIA'
