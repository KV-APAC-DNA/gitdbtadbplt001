    ...
    select inn.*
     ...
     (select distinct ci.*      
      ...
      , ( select employee_key,employee_source_id,
                     us.country_code ,o.organization_key
      ...
      where 
        ...
        and call_date > (select DATE_TRUNC('year', to_date(convert_timezone('UTC',current_timestamp()))-retention_years*365) 
...
(
    ...
    SELECT distinct c1.country_code
        ...
        , C1.CALL_DATE_TIME as utc_call_date_time
        ,C1.LAST_MODIFIED_DATE as utc_call_entry_time
        ,substring( CALL_STATUS_TYPE, 1, length(call_status_type) -4) call_status_type
