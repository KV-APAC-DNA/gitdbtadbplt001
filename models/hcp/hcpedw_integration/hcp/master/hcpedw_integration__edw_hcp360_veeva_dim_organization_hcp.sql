...
final as
(
    SELECT  ORG_HIER.TERRITORY_SOURCE_ID,
    COUNTRY_CODE,
    ...
    -- LEAF NODE
        ...
        (select T.TERRITORY_NAME,
                ...
                T.COUNTRY_CODE
        from ITG_HCP360_VEEVA_TERRITORY T,
                ITG_HCP360_VEEVA_OBJECT_TERRITORY_ASSOCIATION TL
        WHERE T.TERRITORY_SOURCE_ID = TL.TERRITORY2ID
            AND T.COUNTRY_CODE ='IN' ),
        ITG_TERRITORY AS (SELECT * FROM  ITG_HCP360_VEEVA_TERRITORY T WHERE territory_name not in ('ASEANHK','SIMP')),
        LEAF_NODE AS (SELECT * FROM BASE_USER),
                LEVEL_5 AS (SELECT *  FROM ITG_TERRITORY WHERE (TERRITORY_SOURCE_ID)  IN (SELECT PARENT_TERRITORY_SOURCE_ID  FROM BASE_USER)),
        LEVEL_4 AS (SELECT *  FROM ITG_TERRITORY WHERE (TERRITORY_SOURCE_ID)  IN (SELECT PARENT_TERRITORY_SOURCE_ID  FROM LEVEL_5)),          
        LEVEL_3 AS (SELECT *  FROM ITG_TERRITORY WHERE (TERRITORY_SOURCE_ID)  IN (SELECT PARENT_TERRITORY_SOURCE_ID  FROM LEVEL_4)),
        LEVEL_2 AS (SELECT *  FROM ITG_TERRITORY  WHERE (TERRITORY_SOURCE_ID)  IN (SELECT PARENT_TERRITORY_SOURCE_ID  FROM LEVEL_3)),
        LEVEL_1 AS (SELECT *  FROM ITG_TERRITORY  WHERE (TERRITORY_SOURCE_ID)  IN (SELECT PARENT_TERRITORY_SOURCE_ID  FROM LEVEL_2))
        SELECT DISTINCT 
            ...
            LEAF_NODE.TERRITORY_NAME AS  LEVEL_6_TERRITORY_NAME,
                        LEVEL_5.TERRITORY_SOURCE_ID LEVEL_5_TERRITORY_CODE,
            LEVEL_5.TERRITORY_NAME     LEVEL_5_TERRITORY_NAME,
    ...
    (WITH BASE_USER
        ...
        SELECT DISTINCT 
            ...
            LEVEL_5.TERRITORY_SOURCE_ID AS LEVEL_4_TERRITORY_CODE,
            LEVEL_5.TERRITORY_NAME AS  LEVEL_4_TERRITORY_NAME,                 
    ...
    (WITH BASE_USER
        ...
        ITG_TERRITORY AS (SELECT * FROM  ITG_HCP360_VEEVA_TERRITORY T WHERE territory_name not in ('ASEANHK','SIMP')),       
        LEAF_NODE AS (SELECT * FROM BASE_USER),
        LEVEL_4 AS (SELECT *  FROM ITG_TERRITORY WHERE (TERRITORY_SOURCE_ID)  IN (SELECT PARENT_TERRITORY_SOURCE_ID  FROM BASE_USER)),          
        LEVEL_3 AS (SELECT *  FROM ITG_TERRITORY WHERE (TERRITORY_SOURCE_ID)  IN (SELECT PARENT_TERRITORY_SOURCE_ID  FROM LEVEL_4)),
        LEVEL_2 AS (SELECT *  FROM ITG_TERRITORY  WHERE (TERRITORY_SOURCE_ID)  IN (SELECT PARENT_TERRITORY_SOURCE_ID  FROM LEVEL_3))
        SELECT DISTINCT 
            BASE_USER.TERRITORY_NAME,
