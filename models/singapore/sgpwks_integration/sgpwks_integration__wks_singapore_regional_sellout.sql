with sources as (
  select * from {{ source('sgpitg_integration', 'itg_sg_pos_sales_fact_name') }}
),
itg_mds_ap_customer360_config as (
  select * from {{ source('source_name', 'itg_mds_ap_customer360_config ') }}
),
edw_vw_os_time_dim as (
  select * from {{ source('source_name', 'edw_vw_os_time_dim') }}
),
...
edw_customer_dim as (
  select * from {{ source('idnedw_integration','edw_customer_dim') }}
),
final as (
select
  ...
  end as pka_product_key_description::varchar(500),
...
from (
  ...
  trim(coalesce(nullif(cust.region, ''), 'NA')) as region,
  trim(coalesce(nullif(cust.zone_or_area, ''), 'NA')) as zone_or_area,
  trim(coalesce(nullif(cust.cust_segmt_key, ''), 'NA')) as customer_segment_key,
  ...
  cast('sgd' as varchar) /* trim(nvl (nullif(product.pka_product_key,''),'NA')) as pka_product_key, */ /* trim(nvl (nullif(product.pka_product_key_description,''),'NA')) as pka_product_key_description, */ /* trim(nvl (nullif(product.sls_org,''),'NA')) as sls_org, */ as from_currency,
  'usd' as to_currency,
  c.exch_rate as exchange_rate,
  sum(so_sls_qty) as sellout_sales_quantity,
  sum(so_sls_value) as sellout_sales_value,
  sum(so_sls_value * c.exch_rate) as sellout_sales_value_usd
  from wks_Singapore_regioNAl_sellout_base 
  left join (
  select distinct
  cal_year as year,
  ...
  from (
    select
    *,
    row_number() over (partition by matl_num order by matl_num asc) as rank
    from edw_vw_greenlight_skus
    /* sls_org = '2210' */
    where
    ctry_group = 'Singapore' and ctry_nm = 'Singapore'
  )
  where
    rank = 1
  ) as emd, rg_edw.edw_gch_producthierarchy as egph
  where
  emd.matl_num = ltrim(egph.materialnumber(), 0)
  and emd.prod_hier_cd <> ''
  and ltrim(emd.matl_num, '0') in (
    select distinct
    ltrim(matl_num, '0')
    from rg_edw.edw_material_sales_dim
    where
    sls_org /* sls_org = '2210' */ in (
    select distinct
      sls_org
    from edw_vw_greenlight_skus
    where
  ...
  *
  ...
  end, /* product.pka_product_key, */ /* product.pka_product_key_description, */
  case
  when not trim(coalesce(nullif(sellout.pka_product_key, ''), 'NA')) in ('N/A', 'NA')
  then trim(coalesce(nullif(sellout.pka_product_key_description, ''), 'NA'))
  when not trim(coalesce(nullif(product.pka_product_key, ''), 'NA')) in ('N/A', 'NA')
