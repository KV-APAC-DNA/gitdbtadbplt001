...
vst AS (
    ...
    WHERE
        (
            upper((edw_vw_pop6_visits_sku_audits.cntry_cd)::TEXT)
            = ('SG'::CHARACTER VARYING)::TEXT
        )
),
...
ct1 AS (
    SELECT
        ...
        CASE
            WHEN (
                ...
                OR (
                    upper((vst.response)::TEXT) = ((1)::CHARACTER VARYING)::TEXT
                )
            )
        ...
        NULL AS acc_rej_reason,
         datediff(
            SECOND,
            to_date(vst.check_in_datetime::VARCHAR),
            to_date(vst.check_out_datetime::VARCHAR)
        ) AS duration
    FROM (
        (
                (
                        ...
                        SELECT DISTINCT
                            ...
                            (edw_vw_ps_weights.weight)::NUMERIC(20, 4)
                                AS weight,
                            edw_vw_ps_weights.channel
                        FROM edw_vw_ps_weights
                        WHERE (
                            (
                                upper((edw_vw_ps_weights.kpi)::TEXT)
                                = ('MSL COMPLIANCE'::CHARACTER VARYING)::TEXT
                            )
                            AND (
                                upper((edw_vw_ps_weights.market)::TEXT)
                        ...
                        (
                            (
                                ...
                                = upper((kpi_wt.retail_environment)::TEXT)
                            )
                            AND (
                                upper((kpi_wt.channel)::TEXT)
            ...
            LEFT JOIN
                edw_vw_pop6_salesperson AS usr
                ON (((vst.username)::TEXT = (usr.username)::TEXT))
        )
        LEFT JOIN
            edw_vw_pop6_products AS prd
            ON (((vst.sku_id)::TEXT = (prd.productdb_id)::TEXT))
    )
    WHERE (
        (upper((vst.field_code)::TEXT) = ('PS_MSL'::CHARACTER VARYING)::TEXT)
        AND (
            date_part(YEAR, (vst.visit_date)::TIMESTAMP WITHOUT TIME ZONE)
            >= (
                date_part(
                    YEAR,
                    convert_timezone(
    ...
    SELECT
        ...
        CASE
            WHEN (
                ...
                OR (
                    upper((vst.response)::TEXT) = ((0)::CHARACTER VARYING)::TEXT
                )
            )
                THEN (
                    coalesce(CASE
    ...
    FROM (
        (
                        ...
                        (
                                ...
                                (
                                    (
                                        upper((pop.retail_environment_ps)::TEXT)
                                        = upper(
                                            (kpi_wt.retail_environment)::TEXT
                                        )
                                    )
                                    AND (
                                        upper((kpi_wt.channel)::TEXT)
            ...
            WHERE
                (
                    upper((edw_vw_pop6_visits_sku_audits.field_code)::TEXT)
                    = ('PS_MSL_OOS_REASON'::CHARACTER VARYING)::TEXT
                )
        ) AS vst_oos ON (
            (
                (
    ...
    WHERE (
        (
            upper((vst.field_code)::TEXT)
            = ('PS_MSL_OOS'::CHARACTER VARYING)::TEXT
        )
        AND (
            date_part(YEAR, (vst.visit_date)::TIMESTAMP WITHOUT TIME ZONE)
            >= (
    ...
    FROM (
        (
                (
                                                    ...
                                                    WHEN
                                                        (
                                                            upper(
                                                                ...
                                                                '%PS_PROMO%'::CHARACTER VARYING
                                                            )::TEXT
                                                        )
                                                        THEN
                                                            'Promo Compliance'::CHARACTER VARYING
                                                    WHEN
                                                        (
                                                            upper(
                                                                (
                                                                ...
                                                                '%PS_DISPLAY%'::CHARACTER VARYING
                                                            )::TEXT
                                                        )
                                                        THEN
                                                            'Display Compliance'::CHARACTER VARYING
                                                    ELSE NULL::CHARACTER VARYING
                                                END AS kpi,
                                                CASE
                                                    WHEN
                        ...
                        SELECT DISTINCT
                            NULL::CHARACTER VARYING AS brand_l4,
                            (
                                upper((edw_vw_pop6_products.ps_category)::TEXT)
                        ...
                        CASE
                            WHEN
                                ...
                                THEN
                                    (
                                        (vst.product_attribute_value)::TEXT
                                        = (prd.ps_categorysegement)::TEXT
                                    )
                            ELSE
                                (
                                    (vst.product_attribute_value)::TEXT
                ...
                WHERE
                    (
                        upper((edw_vw_pop6_visits_display.field_type)::TEXT)
                ...
                (
                    (
                        ...
                        AND (
                            (vst.product_attribute_value)::TEXT
                            = (rej.product_attribute_value)::TEXT
                        )
                    )
                    AND (
            ...
            WHERE (
                (
                    (
                        upper((edw_vw_ps_weights.kpi)::TEXT)
                        = ('PROMO COMPLIANCE'::CHARACTER VARYING)::TEXT
                    )
                    OR (
                        upper((edw_vw_ps_weights.kpi)::TEXT)
                        = ('DISPLAY COMPLIANCE'::CHARACTER VARYING)::TEXT
                    )
                )
                AND (
                    upper((edw_vw_ps_weights.market)::TEXT)
        ...
        (
                                                    ...
                                                    WHEN
                                                        (
                                                            upper(
                                                                ...
                                                                '%PS_POG%'::CHARACTER VARYING
                                                            )::TEXT
                                                        )
                                                        THEN
                                                            'Planogram Compliance'::CHARACTER VARYING
                                                    ELSE NULL::CHARACTER VARYING
                                                END AS kpi,
                                                CASE
                                                    WHEN
            ...
            WHERE (
                (
                    ...
                    OR (
                        upper((edw_vw_ps_weights.kpi)::TEXT)
                        = ('PLANOGRAM COMPLIANCE'::CHARACTER VARYING)::TEXT
                    )
                )
                AND (
                    upper((edw_vw_ps_weights.market)::TEXT)
    ...
    SELECT
        ...
        CASE
            WHEN
                (
                    upper((vst.kpi)::TEXT)
                    = ('SOS COMPLIANCE'::CHARACTER VARYING)::TEXT
                )
                THEN 'SHARE OF SHELF'::CHARACTER VARYING
            WHEN
                (
                    upper((vst.kpi)::TEXT)
                    = ('SOA COMPLIANCE'::CHARACTER VARYING)::TEXT
                )
        ...
        CASE
            WHEN
                (
                    upper((vst.field_code)::TEXT) LIKE (
    ...
    FROM (
        (
                                            ...
                                            SELECT DISTINCT
                                                ...
                                                CASE
                                                    ...
                                                    WHEN (
                                                        ...
                                                        = (
                                                            'PS_SOA'::CHARACTER VARYING
                                                        )::TEXT
                                                    )
                                                        THEN
                                                            'SOA Compliance'::CHARACTER VARYING
                                                    ELSE NULL::CHARACTER VARYING
                                                END AS kpi,
                                                CASE
                                                    WHEN
            ...
            WHERE (
                (
                    (
                        upper((edw_vw_ps_targets.kpi)::TEXT)
                        = ('SOS COMPLIANCE'::CHARACTER VARYING)::TEXT
                    )
                    OR (
                        upper((edw_vw_ps_targets.kpi)::TEXT)
                ...
                AND (
                    upper((edw_vw_ps_targets.market)::TEXT)
                    = ('SINGAPORE'::CHARACTER VARYING)::TEXT
                )
            )
        ) AS mkt_share ON (
            (
                (
                    (
                        (upper((vst.kpi)::TEXT) = upper((mkt_share.kpi)::TEXT))
                        AND (
                            upper((prd.ps_category)::TEXT)
                            = upper((mkt_share.category)::TEXT)
                        )
                    )
                    AND (
                        upper((prd.ps_segment)::TEXT)
                        = upper((mkt_share.segment)::TEXT)
                    )
                )
                AND (
