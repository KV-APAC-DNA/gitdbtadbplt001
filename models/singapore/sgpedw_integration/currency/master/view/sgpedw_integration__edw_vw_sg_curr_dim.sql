...
(
    ...
    where
    (
        (cast((edw_crncy_exch.from_crncy) as text) = cast('SGD' as text))
        and (cast((edw_crncy_exch.to_crncy) as text) = cast('USD' as text))
...
(
    select
        case
            ...
            (
                (
                    (cast((coalesce(ce.from_ccy, eotd.from_ccy)) as text) = cast('SGD' as text))
                    or 
                    (
