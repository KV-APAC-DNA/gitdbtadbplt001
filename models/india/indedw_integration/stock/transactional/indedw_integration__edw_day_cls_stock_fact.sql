{{
    ...
    (
        ...
        pre_hook = " {% if is_incremental() %}
                    delete from {{this}} USING {{ ref('indedw_integration__edw_day_cls_stock_fact') }}
                    WHERE {{ ref('indedw_integration__edw_day_cls_stock_fact') }}.distcode = {{this}}.customer_code
                    AND {{ ref('indedw_integration__edw_day_cls_stock_fact') }}.transdate = {{this}}.stock_date
                    AND {{ ref('indedw_integration__edw_day_cls_stock_fact') }}.prdcode = {{this}}.product_code;
                    {% endif %}"
    )
}}
