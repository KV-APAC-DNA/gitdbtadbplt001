{{
    ...
    (
        ...
        pre_hook = ["{% if is_incremental() %}
                    DELETE FROM {{this}}
                    WHERE CASE WHEN (SELECT COUNT(*) FROM {{source('indsdl_raw', 'sdl_rmrpstockprocess_opstk')}}) > 0
                    THEN 1
                    ELSE 0
                    END = 1;
                    {% endif %}",
        ...
        {{source('indsdl_raw', 'sdl_rmrpstockprocess_opstk')}} sdl
        {% endif %}"]
    )
}}

with sdl_rmrpstockprocess_opstk as 
(
    select * from {{source('indsdl_raw', 'sdl_rmrpstockprocess_opstk')}}
),
final as 
(
    select
    ...
    from sdl_rmrpstockprocess_opstk
)
select * from final
