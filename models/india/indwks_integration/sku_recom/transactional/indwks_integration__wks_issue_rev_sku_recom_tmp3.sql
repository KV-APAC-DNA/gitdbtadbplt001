with wks_issue_rev_sku_recom_tmp2 as 
(
    select * from {{ ref('indwks_integration__wks_issue_rev_sku_recom_tmp2') }}
),
wks_issue_rev_edw_dailysales_fact as 
(
    select * from {{ ref('indwks_integration__wks_issue_rev_edw_dailysales_fact') }}
),
final as
(
    SELECT sku.mnth_id,
       CASE WHEN sku.mnth_id <> sales.mth_mm THEN 0 ELSE sales.week END AS week,
       sku.cust_cd,
       sku.retailer_cd,
       sku.mother_sku_cd,
       sku.oos_flag,
       ...
       MAX(CASE
         ...
         WHEN sku.ms_flag::TEXT = '1'::CHARACTER VARYING::TEXT AND COALESCE(sales.achievement_nr_val,0) <= 0::NUMERIC(38,6) THEN '0'::CHARACTER VARYING
       ...
       END) AS hit_ms_flag
    FROM wks_issue_rev_sku_recom_tmp2 sku
    LEFT JOIN wks_issue_rev_edw_dailysales_fact sales
            ON DATEDIFF (month,sales.sales_date::DATE,sku.reco_date::DATE) < 3
            AND DATEDIFF (month,sales.sales_date::DATE,sku.reco_date::DATE) >= 0
    ...
    GROUP BY sku.mnth_id,
       CASE WHEN sku.mnth_id <> sales.mth_mm THEN 0 ELSE sales.week END ,
       sku.cust_cd,
       sku.retailer_cd,
       sku.mother_sku_cd,
       sku.oos_flag,
