...
FROM (SELECT ret.retailer_code,
      ...
      FROM edw_retailer_dim ret
        INNER JOIN edw_customer_dim cd
                ON COALESCE(cd.customer_code,'0'::CHARACTER VARYING)::TEXT = COALESCE(ret.customer_code,'0'::CHARACTER VARYING)::TEXT
               AND cd.active_flag = 'Y'
               AND status_desc = 'Active'
               AND UPPER(ret.Channel_Name) IN ('SUB-STOCKIEST','URBAN SUBD','RURAL WHOLESALE','PHARMA RESELLER SUBD')) rd
WHERE rd.rn = 1
)
select * from final