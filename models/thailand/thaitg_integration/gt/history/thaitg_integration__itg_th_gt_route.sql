...
{{
    config(
        ...
        pre_hook="delete from {{this}} where (coalesce(upper(trim(saleunit)),'N/A')) in (select distinct (coalesce(upper(trim(saleunit)),'N/A')) from dev_dna_load.snaposesdl_raw.sdl_th_gt_route ) AND   UPPER(flag) IN ('I','U');"
    )
}}
