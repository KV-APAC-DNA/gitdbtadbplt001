...
TH_WKS_RPT_RETAIL_EXCELLENCE as(
    select * from {{ ref('thawks_integration__th_wks_rpt_retail_excellence') }}
),
...
transformation as (
    (SELECT FISC_YR,
       ...
       P12M_SALES_FLAG,
       MDP_FLAG,
       CAST(TARGET_COMPLAINCE AS INTEGER) AS TARGET_COMPLAINCE,
       LIST_PRICE,
       TOTAL_SALES_LM,
       TOTAL_SALES_P3M,
       TOTAL_SALES_P6M,
       ...
       MD5(soldto_code||DISTRIBUTOR_CODE||nvl (SELL_OUT_CHANNEL,'soc') ||nvl (SELL_OUT_RE,'soe') ||nvl (CUSTOMER_SEGMENT_DESCRIPTION,'csd') || nvl (SAP_CUSTOMER_CHANNEL_DESCRIPTION,'sccd') ||nvl (SAP_SUB_CHANNEL_DESCRIPTION,'sscd') ||nvl (SAP_PARENT_CUSTOMER_DESCRIPTION,'spscd') || nvl (SAP_BANNER_DESCRIPTION,'sbd') || nvl (SAP_BANNER_FORMAT_DESCRIPTION,'sbfd'))
AS CUSTOMER_AGG_DIM_KEY,
MD5(nvl (GLOBAL_PRODUCT_FRANCHISE,'gpf') || nvl (GLOBAL_PRODUCT_BRAND,'gpb') ||nvl (GLOBAL_PRODUCT_SUB_BRAND,'gpsb') ||nvl (GLOBAL_PRODUCT_SEGMENT,'gps') || nvl (GLOBAL_PRODUCT_SUBSEGMENT,'gpss') ||nvl (GLOBAL_PRODUCT_CATEGORY,'gpc') ||nvl (GLOBAL_PRODUCT_SUBCATEGORY,'gpsc') ||nvl (GLOBAL_PRODUCT_VARIANT,'gpv') ||nvl (GLOBAL_PUT_UP_DESCRIPTION,'gppu') 
 ||nvl (PKA_PRODUCT_KEY,'pkapk') ||nvl (PKA_PRODUCT_KEY_DESCRIPTION,'pkapkd'))
AS PRODUCT_AGG_DIM_KEY,
...
FROM (SELECT MAIN.FISC_YR,
			 ...
			 TRIM(NVL (NULLIF(MAIN.SAP_GO_TO_MDL_KEY,''),'NA')) AS SAP_GO_TO_MDL_KEY,
             UPPER(TRIM(NVL (NULLIF(MAIN.SAP_GO_TO_MDL_DESCRIPTION,''),'NA'))) AS SAP_GO_TO_MDL_DESCRIPTION,
            ...
            MAIN.PKA_FRANCHISE_DESC,
             ...
             SYSDATE() AS Created_date
      FROM TH_WKS_RPT_RETAIL_EXCELLENCE MAIN
      LEFT JOIN (SELECT LTRIM(edw_list_price.material,'0') AS material,
                   ...
                   FROM EDW_LIST_PRICE
                   WHERE edw_list_price.sls_org IN ('2400','2500')) lp
      ---- NEED TO CHECK ON
