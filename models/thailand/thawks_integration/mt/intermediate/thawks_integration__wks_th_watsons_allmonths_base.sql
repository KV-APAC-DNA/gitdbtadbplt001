with source as(
    select * from {{ ref('thawks_integration__wks_th_watsons_base') }}
),
        ...
        mnth_id
         from 
             ...
             (select distinct cal_year as year,cal_mnth_id as mnth_id
               ...
               where cal_year >= (DATE_PART(YEAR,CURRENT_TIMESTAMP()) -3) 
             ) b
),
transformed as(
    select all_months.sap_prnt_cust_key as sap_parent_customer_key, 
