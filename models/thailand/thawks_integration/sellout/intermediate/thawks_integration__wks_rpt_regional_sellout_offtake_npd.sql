...
from 
  (
    select 
      *, 
      case when first_scan_flag_parent_customer_level_initial = 'Y' 
      AND rn_cus = 1 THEN 'Y' else 'N' end as first_scan_flag_parent_customer_level, 
      case when first_scan_flag_market_level_initial = 'Y' 
      and first_scan_flag_parent_customer_level_initial = 'Y' 
      and rn_mkt = 1 then 'Y' else 'N' end as first_scan_flag_market_level 
    from 
      (
        select 
          *, 
          case when customer_product_min_date > dateadd(
            week, 
            (
              select 
                param_value 
            ...
            customer_min_date
          ) then 'Y' else 'N' end as first_scan_flag_parent_customer_level_initial, 
          case when market_product_min_date > dateadd(
            week, 
            (
              select 
                param_value 
            ...
            market_min_date
          ) then 'Y' else 'N' end as first_scan_flag_market_level_initial 
        from 
          (
            select 
              year, 
              ...
              updt_dttm 
            from 
              wks_thailand_regional_sellout_npd
          )
      )
  )
),
